									...
									<table id="datatables-reponsive" class="table table-striped" style="width:100%">
										...
										<tbody>
											...
											while($row = $reportstable->fetch_assoc()) {
												...
												$theFinalObject[$counter][] = $row["reportedName"];
												$theFinalObject[$counter][] = $row["reportRefId"];
												$theFinalObject[$counter][] = $row["reportInfo"];
												$theFinalObject[$counter][] = $row["reportedAddress"];
												$theFinalObject[$counter][] = $row["status"];
												$counter++;

											echo '<tr>
												<td id="reportid">' . $row["id"] . '</td>
												<td id="reportedname">' . $row["reportedName"] . '</td>
												<td id="refid">' . $row["reportRefId"] . '</td>
												<td id="extrainfo">' . $row["reportInfo"] . '</td>
												<td id="address">' . $row["reportedAddress"] . '</td>
												<td id="status">' . (($row["status"] == 0)?'<span class="badge bg-warning">Pending</span>':'<span class="badge bg-success">Accepted</span>') . '</td>
												<td id="actions">
  ...
  $('.reportadminbutton').each(function(btn){
        $(this).click(function(){
			...
			setTimeout(function()
			{
				...
				$.get( "changereportstatus.php?reportid="+$(this).parent().siblings("#reportid").text() + "&status=" + statuschange, function( data ) {
...
var head = [['ID', 'Reported Name', 'Test Reference', 'Extra Info', "Address", "Status"]]
