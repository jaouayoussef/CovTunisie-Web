...
$commandestable = $mysqli->query("SELECT * FROM `commandes`") or die("db error");
...
<li class="sidebar-item active"><a class="sidebar-link" href="commandes.php">Commandes</a></li>
						</ul>
					</li>
		...
		<div class="main">
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<div class="mb-3">
						<h1 class="h3 d-inline align-middle">Check reports</h1>
					</div>
						...
						<div class="col-12">
							...
							<div class="card">
								<div class="card-header">
									<h5 class="card-title">Admin tool to check reports</h5>
								</div>
								<div class="card-body">
									<table id="datatables-reponsive" class="table table-striped" style="width:100%">
										...
										<tbody>
											...
											$counter = 0;
											while($row = $commandestable->fetch_assoc()) {
													
												$theFinalObject[$counter][] = $row["id"];
												$theFinalObject[$counter][] = $row["price"];
												$theFinalObject[$counter][] = $row["time"];
												$theFinalObject[$counter][] = $row["nom"];
												$theFinalObject[$counter][] = $row["prenom"];
												$theFinalObject[$counter][] = $row["adresse"];
												$theFinalObject[$counter][] = $row["userid"];
												$theFinalObject[$counter][] = $row["livr"];
												$theFinalObject[$counter][] = $row["etat"];
												$counter++;

											echo '<tr>
												<td id="id">' . $row["id"] . '</td>
												<td id="cart">' . $row["cart"] . '</td>
												...
												<td id="livr">' . $row["livr"] . '</td>
												<td id="status">' . (($row["etat"] == 0)?'<span class="badge bg-warning">Pending</span>':'<span class="badge bg-success">Accepted</span>') . '</td>
												<td id="actions">
													<button id="accept" class="btn btn-success reportadminbutton" action="accept"><i class="fas fa-check"></i></button>
													<button id="decline" class="btn btn-danger reportadminbutton" action="decline"><i class="fas fa-times"></i></button>
													<button id="pend" class="btn btn-warning reportadminbutton" action="pend"><i class="fas fa-exclamation"></i></button>
								...
								<a onclick="generateTable()" class="btn btn-primary">Export pdf</a>
							</div>
						</div>
					</div>
...
<script>
  ...
  $('.reportadminbutton').each(function(btn){
        $(this).click(function(){

			$(this).attr("disabled", true);
			$(this).siblings("#decline").attr("disabled", true);
			$(this).siblings("#accept").attr("disabled", true);
			$(this).siblings("#pend").attr("disabled", true);
			...
			setTimeout(function()
			{
				...
				$.get( "changecommandestatus.php?id="+$(this).parent().siblings("#id").text() + "&status=" + statuschange, function( data ) {

					if(statuschange == -1)
					{
						//$(this).parent().parent().remove();
						$(this).siblings("#decline").remove();
						$(this).siblings("#accept").remove();
						$(this).siblings("#pend").remove();
						$(this).siblings("#loading").remove();
						$(this).parent().siblings("#status").html('<span class="badge bg-danger">Deleted</span>');
						$(this).remove();
					...
					else
					{
						...
						$(this).siblings("#decline").attr("disabled", false);
						$(this).siblings("#accept").attr("disabled", false);
						$(this).siblings("#pend").attr("disabled", false);
						console.log($(this).siblings("#loading").css("display", "none"));

						if(statuschange == 1)
						{
							$(this).parent().siblings("#status").html('<span class="badge bg-success">Accepted</span>');
						}
						else if(statuschange == 0)
						{
							$(this).parent().siblings("#status").html('<span class="badge bg-warning">Pending</span>');
...
var head = [['ID', 'Price', 'Time', "Nom", "Prenom", "Adresse", "User ID", "Livreur", "Etat"]]
var body = <?php echo json_encode($theFinalObject); ?>;
//doc.addImage(myBase64, 'PNG', 15, 40)
...
doc.save('table.pdf');
	...
	/*toDataUrl("../images/logo2.png", function(myBase64) {
