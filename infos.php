
<?php

$mysqli = mysqli_connect("localhost", "root", "", "covtun") or die("0");
$curl = curl_init();



if(!isset($_SESSION["firsttime"]))
{
    $obj = $mysqli->query("UPDATE `infos` SET `value` = `value` + 1 WHERE `infos`.`fieldName` = 'totalVisitors';") or die("0");
    $_SESSION["firsttime"] = true;
}
$obj = $mysqli->query("UPDATE `infos` SET `value` = `value` + 1 WHERE `infos`.`fieldName` = 'visitsCount';") or die("0");

curl_setopt_array($curl, [
	CURLOPT_URL => "https://covid-193.p.rapidapi.com/statistics?country=tunisia",
	CURLOPT_RETURNTRANSFER => true,
	CURLOPT_FOLLOWLOCATION => true,
	CURLOPT_ENCODING => "",
	CURLOPT_MAXREDIRS => 10,
	CURLOPT_TIMEOUT => 30,
	CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
	CURLOPT_CUSTOMREQUEST => "GET",
	CURLOPT_HTTPHEADER => [
		"x-rapidapi-host: covid-193.p.rapidapi.com",
		"x-rapidapi-key: E7HqsZ1JGumsh5ens5AwDwz9M5Y3p1jzpjRjsn0rHDAp8Fz5tH"
	],
]);

$response = curl_exec($curl);
$err = curl_error($curl);

curl_close($curl);

if ($err) {
    echo "cURL Error #:" . $err;
    die();
} else {
	//echo $response;
}






$theData = json_decode($response,true);




$obj = $mysqli->query("SELECT geo_lat, geo_long, prenom, nom FROM `users` WHERE health=1") or die("db error");
$obj2 = $mysqli->query("SELECT reportedLong, reportedLat, status, reportedName FROM `reports`") or die("db error");
while($row = $obj->fetch_assoc()) {
        
        $infectedRows[] = $row;

    }
    while($row2 = $obj2->fetch_assoc()) {
        
        $reportedRows[] = $row2;

    }
    





?>
<!DOCTYPE html>
<html lang="en">

<!-- Mirrored from techydevs.com/demos/themes/html/costar/prevention.html by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 01 Dec 2020 23:03:15 GMT -->
<head>
    <meta http-equiv="content-type" content="text/html; charset=utf-8">
    <meta name="author" content="TechyDevs">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width,  initial-scale=1.0">
    <meta name="description" content="Covtunisie information page">
    <meta name="keywords" content="medical, corona, virus, health, html5 template">

    <title>CovTunisie</title>

    <!-- Favicon -->
    <link rel="icon" sizes="16x16" href="images/favicon.png">

    <!-- Google Fonts -->
    <link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,500&amp;display=swap" rel="stylesheet">



    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.7.1/dist/leaflet.css"
   integrity="sha512-xodZBNTC5n17Xt2atTPuE1HxjVMSvLVW9ocqUKLsCC5CXdbqCmblAshOMAS6/keqq/sMZMZ19scR4PsZChSR7A=="
   crossorigin=""/>

    <!-- inject:css -->
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="css/font-awesome.min.css">
    <link rel="stylesheet" href="css/line-awesome.css">
    <link rel="stylesheet" href="css/animate.min.css">
    <link rel="stylesheet" href="css/owl.carousel.min.css">
    <link rel="stylesheet" href="css/owl.theme.default.min.css">
    <link rel="stylesheet" href="css/magnific-popup.css">
    <link rel="stylesheet" href="css/style.css">
    <!-- end inject -->
</head>
<body>

<!-- page preloader -->
<div id="loading-area">
    <div class="loader-col">
        <div class="loader" id="loader"></div>
    </div>
</div><!-- end loading -->

<!--======================================
        START HEADER AREA
    ======================================-->
<section class="header-menu-area header-menu-area-2">
    <div class="header-menu-fluid">
        <div class="container ">
            <div class="main-menu-content-2">
                <div class="row align-items-center h-100">
                    <div class="col-lg-3">
                        <div class="logo-box">
                            <a href="index.html" class="logo"><img src="images/logo2.png" alt="logo"></a>
                        </div>
                    </div><!-- end col-lg-3 -->
                    <div class="col-lg-9">
                        <div class="menu-wrapper">
                            <nav class="main-menu">
                                <ul>
                                    <li>
                                        <a href="#">Home</a>
                                        <ul class="dropdown-menu-item">
                                            <li><a href="index.html">Home 01</a></li>
                                            <li><a href="home-2.html">Home 02</a></li>
                                            <li><a href="home-3.html">Home 03</a></li>
                                            <li><a href="home-4.html">Home 04</a> <span class="new-page-badge">new</span></li>
                                        </ul>
                                    </li>
                                    <li><a href="about.html">about</a></li>
                                    <li>
                                        <a href="#">pages</a>
                                        <ul class="dropdown-menu-item">
                                            <li><a href="doctors.html">Our doctors</a></li>
                                            <li><a href="doctor-single.html">Doctor detail</a></li>
                                            <li><a href="contact.html">Contact us</a></li>
                                            <li><a href="faq.html">Faqs</a></li>
                                            <li><a href="error.html">Error page</a></li>
                                            <li><a href="recover.html">Recover pass</a></li>
                                        </ul>
                                    </li>
                                    <li><a href="infos.php">Information</a></li>
                                    <li>
                                        <a href="#">blog</a>
                                        <ul class="dropdown-menu-item">
                                            <li><a href="blog-grid.html">blog grid</a></li>
                                            <li><a href="blog-sidebar.html">blog sidebar</a></li>
                                            <li><a href="blog-single.html">blog detail</a></li>
                                        </ul>
                                    </li>
                                    <li>
                                        <a href="login.html">log in</a>
                                        <span class="ml-1 mr-1">/</span>
                                        <a href="sign-up.html">Sign in</a>
                                    </li>
                                </ul><!-- end ul -->
                            </nav><!-- end main-menu -->
                            <div class="logo-right-button logo-right-button-2">
                                <ul>
                                    <li><a href="infos.php" class="template-btn">Information</a></li>
                                </ul>
                                <div class="side-menu-open">
                                    <i class="la la-bars"></i>
                                </div>
                            </div><!-- end logo-right-button -->
                            <div class="side-nav-container">
                                <div class="humburger-menu">
                                    <div class="humburger-menu-lines side-menu-close"></div><!-- end humburger-menu-lines -->
                                </div><!-- end humburger-menu -->
                                <div class="side-menu-wrap">
                                    <ul class="side-menu-ul">
                                        <li class="sidenav__item">
                                            <a href="index.html">home</a>
                                            <span class="menu-plus-icon"></span>
                                            <ul class="side-sub-menu">
                                                <li><a href="index.html">Home 01</a></li>
                                                <li><a href="home-2.html">Home 02</a></li>
                                                <li><a href="home-3.html">Home 03</a></li>
                                                <li><a href="home-4.html">Home 04 <span class="new-page-badge">new</span></a></li>
                                            </ul>
                                        </li>
                                        <li class="sidenav__item"><a href="about.html">about</a></li>
                                        <li class="sidenav__item">
                                            <a href="#">pages</a>
                                            <span class="menu-plus-icon"></span>
                                            <ul class="side-sub-menu">
                                                <li><a href="doctors.html">Our doctors</a></li>
                                                <li><a href="doctor-single.html">Doctor detail</a></li>
                                                <li><a href="contact.html">Contact us</a></li>
                                                <li><a href="faq.html">Faqs</a></li>
                                                <li><a href="error.html">Error page</a></li>
                                                <li><a href="recover.html">Recover pass</a></li>
                                            </ul>
                                        </li>
                                        <li class="sidenav__item"><a href="infos.php">Information</a></li>
                                        <li class="sidenav__item">
                                            <a href="blog-grid.html">blog</a>
                                            <span class="menu-plus-icon"></span>
                                            <ul class="side-sub-menu">
                                                <li><a href="blog-grid.html">blog grid</a></li>
                                                <li><a href="blog-sidebar.html">blog sidebar</a></li>
                                                <li><a href="blog-single.html">blog detail</a></li>
                                            </ul>
                                        </li>
                                        <li class="sidenav__item">
                                            <a href="contact.html">contact</a>
                                        </li>
                                    </ul>
                                    <div class="side-btn-box">
                                        <a href="login.html">log in</a>
                                        <span class="ml-1 mr-1">/</span>
                                        <a href="sign-up.html">Sign in</a>
                                    </div>
                                </div><!-- end side-menu-wrap -->
                            </div><!-- end side-nav-container -->
                        </div><!-- end menu-wrapper -->
                    </div><!-- end col-lg-9 -->
                </div><!-- end row -->
            </div>
        </div><!-- end container -->
    </div><!-- end header-menu-fluid-->
</section><!-- end header-menu-area -->
<!--======================================
        END HEADER AREA
======================================-->

<!-- ================================
    START BREADCRUMB AREA
================================= -->
<section class="breadcrumb-area">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="breadcrumb-content text-center">
                    <div class="section-heading">
                        <h2 class="section__title">Health Information</h2>
                    </div>
                </div><!-- end breadcrumb-content -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
    </div><!-- end container -->
    <div class="breadcrumb-list">
        <ul class="list-items">
            <li class="active__list-item"><a href="index.html">home</a></li>
            <li>Information</li>
        </ul>
    </div>
</section><!-- end breadcrumb-area -->
<!-- ================================
    END BREADCRUMB AREA
================================= -->







<!-- ================================
       START FUNFACT AREA
================================= -->
<section class="funfact-area section--padding text-center">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="section-heading text-center">
                    <h5 class="section__meta">Statistics</h5>
                    <h2 class="section__title padding-bottom-15px line-height-55">Live numbers of<br>
                    Tunisia's Coronavirus state</h2>
                </div><!-- end section-heading -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->

<br>



                <div class="counter-item counter-item-color-5" style="border-right:none;" >
                    <h4 class="counter__title">
                        <span data-purecounter-duration="2.5" data-purecounter-end="<?php echo $theData["response"][0]["population"] ?>" class="counter purecounter">0</span>
                    </h4>
                    <p class="counter__meta">Total population</p>
                </div><!-- end counter-item -->



<div style="width: 500px;
    margin: auto;
  padding: 10px;
    overflow: hidden;">



                <div class="counter-item counter-item-color-1" style="border-right:none;float:left;">
                    <h4 class="counter__title">
                        <span data-purecounter-duration="2.5" data-purecounter-end="<?php echo $theData["response"][0]["cases"]["new"] ?>" class="counter purecounter">0</span>
                    </h4>
                    <p class="counter__meta">New cases today</p>
                </div><!-- end counter-item -->

                <div class="counter-item counter-item-color-1" style="border-right:none;float:right;">
                    <h4 class="counter__title">
                        <span data-purecounter-duration="2.5" data-purecounter-end="<?php echo $theData["response"][0]["deaths"]["new"] ?>" class="counter purecounter">0</span>
                    </h4>
                    <p class="counter__meta">New deaths today</p>
                </div><!-- end counter-item -->



</div>
        <div class="row mt-5">
            <div class="col-lg-3">
                <div class="counter-item counter-item-color-1">
                    <h4 class="counter__title">
                    <span data-purecounter-duration="2.5" data-purecounter-end="<?php echo $theData["response"][0]["cases"]["total"] ?>" class="counter purecounter">0</span>
                    </h4>
                    <p class="counter__meta">Confirmed cases</p>
                </div><!-- end counter-item -->
            </div><!-- end col-lg-3 -->
            <div class="col-lg-3">
                <div class="counter-item counter-item-color-2">
                    <h4 class="counter__title">
                        <span data-purecounter-duration="2.5" data-purecounter-end="<?php echo $theData["response"][0]["deaths"]["total"] ?>" class="counter purecounter">0</span>
                    </h4>
                    <p class="counter__meta">Deaths</p>
                </div><!-- end counter-item -->
            </div><!-- end col-lg-3 -->
            <div class="col-lg-3">
                <div class="counter-item counter-item-color-3">
                    <h4 class="counter__title">
                        <span data-purecounter-duration="2.5" data-purecounter-end="<?php echo $theData["response"][0]["cases"]["recovered"] ?>" class="counter purecounter">0</span>
                    </h4>
                    <p class="counter__meta">Recovered</p>
                </div><!-- end counter-item -->
            </div><!-- end col-lg-3 -->
            <div class="col-lg-3">
                <div class="counter-item counter-item-color-4">
                    <h4 class="counter__title">
                        <span data-purecounter-duration="2.5" data-purecounter-end="<?php echo $theData["response"][0]["cases"]["active"] ?>" class="counter purecounter">0</span>
                    </h4>
                    <p class="counter__meta">Active</p>
                </div><!-- end counter-item -->
            </div><!-- end col-lg-3 -->


            
            
            
        </div><!-- end row -->
        <br>
        <center>
            <p class="copy__desc">Live data for: <br> <?php echo $theData["response"][0]["day"] ?> </p>
            </center>
    </div><!-- end container -->
</section><!-- end funfact-area -->
<!-- ================================
       START FUNFACT AREA
================================= -->




<!--======================================
        START SERVICE AREA
======================================-->
<section class="service-area padding-top-140px padding-bottom-90px text-center">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="section-heading">
                    <h5 class="section__meta">Virus Symptoms</h5>
                    <h2 class="section__title padding-bottom-15px">Coronavirus Symptoms</h2>
                    <div class="section-divider"></div>
                </div><!-- end section-heading -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
        <div class="row mt-5">
            <div class="col-lg-4 column-td-6">
                <div class="info-box">
                    <div class="info__icon mx-auto">
                        <svg class="info_svg" id="Capa_1" enable-background="new 0 0 512 512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><g><path d="m132.047 418.136c-3.905-3.905-10.236-3.905-14.141 0l-37.897 37.898c-3.905 3.905-3.905 10.236 0 14.141 1.952 1.952 4.511 2.928 7.07 2.928s5.118-.977 7.071-2.929l37.897-37.898c3.905-3.903 3.905-10.235 0-14.14z"/><path d="m105.551 392.94c-1.911-5.182-7.664-7.831-12.842-5.92l-47.982 17.699c-5.181 1.911-7.832 7.661-5.92 12.842 1.49 4.04 5.314 6.542 9.382 6.542 1.149 0 2.319-.2 3.46-.621l47.982-17.699c5.18-1.912 7.832-7.662 5.92-12.843z"/><path d="m157.243 444.633c-5.181-1.91-10.931.739-12.842 5.921l-17.698 47.983c-1.911 5.181.74 10.93 5.921 12.842 1.141.421 2.31.621 3.459.621 4.068 0 7.893-2.502 9.383-6.542l17.698-47.983c1.911-5.182-.74-10.931-5.921-12.842z"/><path d="m394.087 418.137c-3.905-3.905-10.236-3.905-14.141 0s-3.905 10.236 0 14.141l37.897 37.898c1.953 1.953 4.511 2.929 7.071 2.929 2.559 0 5.118-.977 7.07-2.928 3.905-3.905 3.905-10.236 0-14.141z"/><path d="m467.292 404.717-48.01-17.699c-5.182-1.909-10.93.742-12.841 5.923-1.91 5.182.742 10.93 5.923 12.841l48.01 17.699c1.141.42 2.309.62 3.458.62 4.069 0 7.893-2.503 9.383-6.543 1.91-5.181-.741-10.93-5.923-12.841z"/><path d="m367.622 450.559c-1.908-5.182-7.656-7.837-12.838-5.928-5.182 1.908-7.836 7.656-5.928 12.838l17.67 47.983c1.489 4.043 5.314 6.547 9.384 6.547 1.147 0 2.315-.199 3.454-.619 5.182-1.908 7.836-7.656 5.928-12.838z"/><path d="m369.564 388.314c1.427.707 2.942 1.043 4.433 1.043 3.68 0 7.222-2.04 8.967-5.561l.028-.057c2.453-4.948.416-10.919-4.532-13.372-4.949-2.451-10.962-.401-13.414 4.547s-.43 10.947 4.518 13.4z"/><path d="m348.161 402.834c-3.146-4.539-9.376-5.668-13.915-2.521-12.549 8.699-25.572 15.573-38.732 20.439-14.811 5.521-27.368 8.092-39.518 8.092s-24.707-2.571-39.531-8.097c-59.793-22.196-112.88-85.424-121.767-141.63-.035-.356-.09-.706-.162-1.05-.753-5.069-1.151-10.079-1.151-14.993v-57.104c23.995-22.737 41.242-30.486 67.584-37.899 13.865-3.884 20.362-1.957 33.304 1.882 3.222.956 6.875 2.039 10.954 3.107 87.501 22.892 122.471 26.957 192.14 35.055 6.724.782 13.781 1.602 21.239 2.48v52.479c0 23.056-8.971 49.876-25.259 75.521-2.961 4.661-1.582 10.841 3.08 13.802 1.662 1.056 3.517 1.56 5.352 1.56 3.307 0 6.545-1.641 8.45-4.64 12.375-19.483 20.983-39.796 25.264-59.198h6.692c29.889 0 46.016-27.612 46.016-53.592 0-11.392-3.041-22.012-8.562-29.904-6.728-9.616-16.871-14.912-28.561-14.912h-13.498l-5.31-48.964c-1.717-16.031-.644-17.705 4.718-22.978 2.063-2.033 4.394-4.066 6.862-6.219 12.855-11.213 30.459-26.569 32.747-64.189.194-3.188-1.147-6.278-3.609-8.313s-5.747-2.771-8.844-1.982c-5.353 1.366-10.266 2.688-14.601 3.855-31.799 8.56-39.355 10.594-69.354-7.905-49.052-30.244-93.014-40.741-134.396-32.093-35.139 7.344-60.694 26.88-81.228 42.577-1.651 1.262-3.254 2.487-4.816 3.671-16.714-4.624-33.652-2.49-48.219 6.189-17.465 10.405-29.793 29.458-32.977 50.953-4.796 32.086-5.14 44.471-5.164 85.4h-12.446c-11.695 0-21.844 5.296-28.576 14.911-5.526 7.893-8.569 18.515-8.569 29.909 0 25.979 16.135 53.589 46.037 53.589h6.682c5.566 25.114 18.384 51.701 37.013 76.015 26.052 34.003 60.137 60.057 95.963 73.356 17.117 6.38 31.893 9.351 46.503 9.351s29.387-2.971 46.478-9.342c14.712-5.441 29.235-13.096 43.165-22.752 4.539-3.147 5.668-9.377 2.522-13.916zm90.445-139.758v-51.365h12.472c5.19 0 9.173 2.086 12.175 6.378 3.146 4.496 4.95 11.217 4.95 18.44 0 13.503-6.929 33.593-26.017 33.593h-3.81c.149-2.375.23-4.726.23-7.046zm-340.272-153.821c2.314-15.628 11.074-29.353 23.431-36.716 7.675-4.572 20.225-8.784 36.436-2.882 3.154 1.147 6.668.638 9.365-1.355 2.955-2.184 5.961-4.482 9.143-6.915 39.217-29.98 92.928-71.039 192.984-9.348 37.35 23.032 52.298 19.008 85.048 10.194.005-.001.011-.003.017-.005-4.153 18.892-13.872 27.371-24.054 36.252-2.585 2.255-5.257 4.586-7.746 7.039-12.059 11.86-12.48 21.549-10.572 39.372l4.925 45.417c-6.139-.72-12.001-1.402-17.635-2.056-68.741-7.991-103.245-12.001-189.387-34.538-3.766-.985-7.102-1.975-10.329-2.932-8.704-2.582-16.265-4.826-25.443-4.826-5.531 0-11.65.815-18.957 2.863-23.819 6.703-41.63 13.985-62.151 30.519.125-30.986.835-42.718 4.925-70.083zm-28.499 160.867c-19.104 0-26.038-20.088-26.038-33.59 0-7.222 1.806-13.942 4.953-18.439 3.007-4.294 6.995-6.381 12.193-6.381h12.443v51.365c0 2.321.085 4.673.236 7.046h-3.787z"/><path d="m225.287 391.059h61.447c15.413 0 27.954-12.553 27.954-27.982 0-32.363-26.329-58.693-58.691-58.693s-58.692 26.329-58.692 58.693c0 15.429 12.553 27.982 27.982 27.982zm30.71-66.677c21.335 0 38.693 17.358 38.693 38.694 0 4.402-3.569 7.984-7.955 7.984h-61.447c-4.327 0-7.983-3.656-7.983-7.984-.002-21.336 17.356-38.694 38.692-38.694z"/><path d="m356.043 292.764c3.597 0 7.074-1.947 8.862-5.354 2.567-4.89.684-10.934-4.206-13.501l-25.293-13.277 25.293-13.277c4.89-2.567 6.773-8.611 4.206-13.501s-8.612-6.773-13.501-4.206l-42.158 22.131c-3.291 1.727-5.352 5.137-5.352 8.853 0 3.717 2.061 7.126 5.352 8.853l42.158 22.131c1.483.779 3.072 1.148 4.639 1.148z"/><path d="m147.087 287.408c1.788 3.408 5.264 5.356 8.863 5.356 1.565 0 3.155-.369 4.637-1.146l42.187-22.131c3.292-1.727 5.354-5.137 5.354-8.855 0-3.717-2.062-7.128-5.354-8.855l-42.187-22.131c-4.889-2.565-10.934-.681-13.5 4.21-2.565 4.89-.681 10.935 4.21 13.5l25.307 13.276-25.307 13.276c-4.891 2.565-6.776 8.61-4.21 13.5z"/></g></svg>
                    </div>
                    <h3 class="info__title">Coughing And Sneezing</h3>
                    <p class="info__text">
                        This is one of the most common symptoms but doesn't necessarily indicate Corona-Virus
                    </p>
                </div><!-- end info-box -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 column-td-6">
                <div class="info-box">
                    <div class="info__icon mx-auto">
                        <svg class="info_svg" id="Capa_2" enable-background="new 0 0 512.375 512.375" viewBox="0 0 512.375 512.375" xmlns="http://www.w3.org/2000/svg"><g><path d="m372.906 253.482h7.146c19.392 0 35.168-15.776 35.168-35.168 0-7.881-2.605-15.164-7-21.034v-83.255c.001-62.874-51.151-114.025-114.025-114.025h-85.97c-42.712 0-77.854 33.037-81.22 74.897-12.521 3.224-21.802 14.61-21.802 28.124v24.66c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-24.66c0-7.74 6.297-14.037 14.038-14.037 4.142 0 7.5-3.357 7.5-7.5 0-36.659 29.825-66.484 66.484-66.484h85.97c54.603 0 99.025 44.423 99.025 99.025v71.688c-4.07-1.65-8.513-2.567-13.168-2.567h-6.68v-33.997c0-13.56-6.146-26.102-16.86-34.41-10.715-8.31-24.394-11.139-37.524-7.762l-63.004 16.197c-.512.131-1.053.129-1.567-.004l-.634-.165c-4.009-1.044-8.104 1.359-9.148 5.368-1.044 4.008 1.359 8.104 5.368 9.147l.631.164c2.972.775 6.113.782 9.084.017l63.004-16.197c8.607-2.214 17.574-.358 24.598 5.088s11.052 13.668 11.052 22.557v93.926c0 56.459-45.933 102.392-102.391 102.392-18.173 0-36.027-4.827-51.632-13.958-3.575-2.094-8.168-.89-10.261 2.685-2.092 3.575-.89 8.169 2.685 10.261 5.502 3.22 11.248 5.968 17.171 8.237v6.229l-80.431 44.356c-13.996 7.718-22.691 22.441-22.691 38.425v63.174c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-63.174c0-10.52 5.722-20.21 14.935-25.29l44.145-24.345c5.554 33.788 34.949 59.652 70.285 59.652 11.402 0 22.325-2.644 32.465-7.856 3.684-1.894 5.135-6.415 3.241-10.099s-6.415-5.138-10.1-3.241c-7.998 4.111-16.613 6.196-25.606 6.196-29.744 0-54.154-23.213-56.102-52.473l14.86-8.195v4.425c0 22.741 18.501 41.242 41.243 41.242s41.243-18.501 41.243-41.242v-4.425l14.869 8.2c-.653 9.774-3.834 19.167-9.307 27.364-2.3 3.444-1.372 8.102 2.073 10.401 1.28.854 2.727 1.264 4.158 1.264 2.422 0 4.799-1.171 6.245-3.337 5.655-8.471 9.373-17.95 11.006-27.875l44.145 24.345c9.211 5.08 14.934 14.771 14.934 25.29v63.174c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-63.174c0-15.983-8.694-30.707-22.69-38.425l-80.432-44.357v-5.637c41.744-15.372 72.415-53.824 76.474-99.802zm.878-55.336h6.268c11.121 0 20.168 9.048 20.168 20.169s-9.047 20.168-20.168 20.168h-6.268zm-92.354 182.329c0 14.47-11.772 26.242-26.243 26.242s-26.243-11.772-26.243-26.242v-23.16c8.818 2.083 17.893 3.151 27.037 3.151 8.737 0 17.251-.967 25.448-2.786v22.795z"/><path d="m139.053 253.482c.478 5.434 1.326 10.821 2.547 16.104.801 3.469 3.887 5.814 7.301 5.814.559 0 1.127-.063 1.695-.194 4.036-.932 6.552-4.96 5.62-8.995-1.741-7.538-2.624-15.322-2.624-23.137v-52.429-41.329c0-10.062 5.15-19.192 13.776-24.426 3.542-2.148 4.671-6.761 2.523-10.302-2.148-3.542-6.761-4.671-10.302-2.522-13.147 7.975-20.997 21.9-20.997 37.25v33.829h-6.268c-4.257 0-8.34.761-12.12 2.153v-25.654c0-4.143-3.358-7.5-7.5-7.5s-7.5 3.357-7.5 7.5v36.303c-5.026 6.083-8.049 13.878-8.049 22.367 0 19.392 15.776 35.168 35.168 35.168zm-26.899-35.168c0-11.121 9.047-20.169 20.168-20.169h6.268v40.337h-6.268c-11.12 0-20.168-9.046-20.168-20.168z"/><path d="m204.096 200.085c-4.142 0-7.5 3.357-7.5 7.5v8.729c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-8.729c0-4.143-3.358-7.5-7.5-7.5z"/><path d="m174.626 190.003-.146.261c-2.024 3.614-.734 8.185 2.88 10.208 1.159.649 2.417.957 3.657.957 2.626 0 5.176-1.382 6.551-3.837l.145-.258c3.841-6.852 10.431-11.802 18.082-13.581 4.034-.938 6.544-4.969 5.606-9.003-.938-4.035-4.97-6.55-9.004-5.606-11.749 2.731-21.87 10.333-27.771 20.859z"/><path d="m299.779 216.314c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-8.729c0-4.143-3.358-7.5-7.5-7.5s-7.5 3.357-7.5 7.5z"/><path d="m323.661 197.331.146.261c1.375 2.455 3.924 3.837 6.551 3.837 1.24 0 2.498-.308 3.657-.957 3.614-2.023 4.904-6.594 2.88-10.208l-.148-.264c-5.898-10.523-16.02-18.125-27.769-20.856-4.038-.946-8.066 1.572-9.004 5.606s1.572 8.065 5.606 9.003c7.651 1.779 14.241 6.729 18.081 13.578z"/><path d="m315.11 292.477c12.502 0 22.674-10.172 22.674-22.674 0-3.509-1.093-9.733-10.539-23.214-2.77-3.952-7.306-6.313-12.135-6.313s-9.365 2.36-12.134 6.312c-9.447 13.48-10.54 19.705-10.54 23.215.001 12.502 10.172 22.674 22.674 22.674zm0-37.066c7.407 10.614 7.668 14.291 7.674 14.397-.003 4.229-3.444 7.668-7.674 7.668-4.231 0-7.674-3.442-7.674-7.665.002-.036.192-3.679 7.674-14.4z"/><path d="m207.301 97.854c-4.761 0-9.233 2.326-11.963 6.223-9.212 13.146-10.278 19.237-10.278 22.676 0 12.264 9.978 22.241 22.241 22.241 12.264 0 22.242-9.978 22.242-22.241 0-3.438-1.065-9.529-10.278-22.677-2.73-3.896-7.203-6.222-11.964-6.222zm.001 36.139c-3.993 0-7.241-3.248-7.241-7.226.001-.034.217-3.507 7.242-13.624 6.922 9.972 7.232 13.488 7.241 13.62-.007 3.988-3.253 7.23-7.242 7.23z"/><path d="m159.388 286.825c-8.703 12.419-9.709 18.221-9.709 21.505 0 11.746 9.556 21.302 21.302 21.302s21.302-9.556 21.302-21.302c0-3.284-1.006-9.086-9.709-21.506-2.646-3.775-6.98-6.029-11.594-6.029-4.613 0-8.947 2.255-11.592 6.03zm17.895 21.534c-.016 3.461-2.837 6.272-6.302 6.272-3.475 0-6.302-2.827-6.303-6.263.002-.031.263-3.126 6.303-11.938 5.869 8.565 6.281 11.729 6.302 11.929z"/><path d="m226.621 309.361v4.479c0 9.183 7.471 16.653 16.654 16.653h23.825c9.183 0 16.654-7.471 16.654-16.653v-4.479c0-15.752-12.815-28.566-28.566-28.566s-28.567 12.814-28.567 28.566zm15 0c0-7.48 6.086-13.566 13.566-13.566s13.566 6.086 13.566 13.566v4.479c0 .912-.742 1.653-1.654 1.653h-23.825c-.912 0-1.654-.741-1.654-1.653v-4.479z"/><path d="m249.57 251.323.192-.324c1.506-2.549 3.971-2.769 4.949-2.778.986.014 3.436.325 4.87 2.915 1.369 2.473 3.928 3.868 6.567 3.867 1.229 0 2.476-.303 3.627-.94 3.624-2.007 4.934-6.571 2.926-10.194-3.634-6.562-10.282-10.541-17.782-10.646-7.519-.134-14.255 3.689-18.068 10.14l-.188.318c-2.11 3.564-.932 8.165 2.633 10.275 3.563 2.108 8.163.932 10.274-2.633z"/><path d="m169.884 471.731c-4.142 0-7.5 3.357-7.5 7.5v25.644c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-25.644c0-4.142-3.358-7.5-7.5-7.5z"/><path d="m340.491 471.731c-4.142 0-7.5 3.357-7.5 7.5v25.644c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-25.644c0-4.142-3.358-7.5-7.5-7.5z"/></g></svg>
                    </div>
                    <h3 class="info__title">High Fever</h3>
                    <p class="info__text">
                        A body temperature of 103°F (39.4°C) or above.
                    </p>
                </div><!-- end info-box -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 column-td-6">
                <div class="info-box">
                    <div class="info__icon mx-auto">
                        <svg class="info_svg" id="Capa_3" enable-background="new 0 0 512.375 512.375" viewBox="0 0 512.375 512.375" xmlns="http://www.w3.org/2000/svg"><g><path d="m408.72 113.995c0-4.143-3.357-7.484-7.5-7.484s-7.5 3.372-7.5 7.515v71.688c-4.07-1.65-8.513-2.567-13.169-2.567h-6.679v-21.787c0-4.143-3.357-7.5-7.5-7.5s-7.5 3.357-7.5 7.5v81.714c0 56.459-45.933 102.392-102.392 102.392-56.458 0-102.391-45.933-102.391-102.392v-52.429-53.54c33.465.97 65.538-7.884 95.502-26.403l.257 5.635c.098 2.139 1.105 4.135 2.768 5.483 1.664 1.349 3.824 1.922 5.937 1.576 12.112-1.985 23.543-6.291 34.075-12.824l.815 8.201c.391 3.937 3.755 6.869 7.728 6.753 19.036-.671 38.418-4.361 57.7-10.977v14.849c0 4.143 3.357 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-21.313c0-3.432-1.685-6.646-4.506-8.601-2.816-1.95-6.408-2.395-9.611-1.191-17.412 6.545-34.901 10.521-52.096 11.847l-.858-8.632c-.37-3.732-2.731-7.001-6.161-8.529-3.414-1.523-7.398-1.095-10.406 1.115-7.979 5.867-16.629 10.11-25.806 12.669l-.212-4.654c-.172-3.77-2.373-7.164-5.743-8.859-3.37-1.696-7.409-1.439-10.537.67-30.237 20.369-62.824 29.814-96.875 28.051-3.089-.149-6.168.974-8.421 3.112-2.256 2.143-3.55 5.155-3.55 8.268v49.795h-6.268c-4.257 0-8.34.761-12.12 2.153v-72.348c0-4.143-3.358-7.5-7.5-7.5s-7.5 3.357-7.5 7.5v82.996c-5.026 6.083-8.049 13.878-8.049 22.367 0 19.392 15.776 35.168 35.168 35.168h6.735c4.007 45.391 33.955 83.442 74.887 99.195v6.243l-80.431 44.356c-13.996 7.718-22.691 22.441-22.691 38.425v63.174c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-63.174c0-10.52 5.722-20.21 14.935-25.29l44.143-24.344c1.642 9.984 5.392 19.513 11.101 28.02 1.447 2.156 3.818 3.321 6.234 3.321 1.437 0 2.89-.412 4.173-1.273 3.439-2.309 4.355-6.969 2.047-10.407-5.519-8.222-8.724-17.653-9.38-27.478l14.868-8.2v4.425c0 22.741 18.501 41.242 41.243 41.242s41.243-18.501 41.243-41.242v-4.425l14.859 8.194c-1.948 29.26-26.358 52.474-56.102 52.474-9.011 0-17.643-2.093-25.654-6.221-3.682-1.898-8.205-.45-10.102 3.231-1.897 3.683-.45 8.205 3.232 10.103 10.157 5.233 21.099 7.887 32.524 7.887 35.336 0 64.731-25.864 70.284-59.652l44.146 24.346c9.211 5.08 14.934 14.771 14.934 25.29v63.174c0 4.143 3.357 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-63.174c0-15.983-8.694-30.707-22.69-38.425l-80.428-44.357v-5.638c41.746-15.37 72.417-53.822 76.476-99.8h7.146c19.393 0 35.169-15.776 35.169-35.168 0-7.881-2.605-15.164-7-21.034v-83.285zm-296.066 104.319c0-11.121 9.047-20.169 20.168-20.169h6.268v40.337h-6.268c-11.121 0-20.168-9.046-20.168-20.168zm143.033 188.403c-14.47 0-26.243-11.772-26.243-26.242v-23.157c8.683 2.055 17.735 3.148 27.037 3.148 8.737 0 17.251-.967 25.449-2.787v22.795c0 14.47-11.772 26.243-26.243 26.243zm124.865-168.235h-6.268v-40.337h6.268c11.121 0 20.169 9.048 20.169 20.169s-9.048 20.168-20.169 20.168z"/><path d="m148.312 38.146c1.427 1.261 3.199 1.88 4.963 1.88 2.075 0 4.141-.856 5.623-2.534 12.632-14.294 30.793-22.492 49.827-22.492h85.97c22.485 0 43.679 7.347 61.291 21.245 3.253 2.566 7.967 2.01 10.534-1.241 2.565-3.252 2.01-7.968-1.241-10.534-20.286-16.008-44.693-24.47-70.584-24.47h-85.97c-23.332 0-45.59 10.045-61.066 27.561-2.743 3.103-2.451 7.842.653 10.585z"/><path d="m250.07 254.323.192-.324c1.506-2.55 3.969-2.818 4.949-2.778.986.014 3.435.325 4.87 2.915 1.369 2.472 3.929 3.867 6.567 3.867 1.229 0 2.476-.303 3.627-.94 3.623-2.007 4.934-6.57 2.927-10.194-3.634-6.562-10.281-10.542-17.781-10.646-7.505-.083-14.256 3.688-18.069 10.141l-.188.318c-2.11 3.564-.932 8.165 2.633 10.275 3.563 2.108 8.163.931 10.273-2.634z"/><path d="m225.421 312.394h65.01c4.143 0 7.5-3.357 7.5-7.5s-3.357-7.5-7.5-7.5h-65.01c-4.142 0-7.5 3.357-7.5 7.5s3.358 7.5 7.5 7.5z"/><path d="m185.925 227.279c1.472 1.981 3.734 3.027 6.026 3.027 1.555 0 3.124-.481 4.467-1.479l8.242-6.124 8.242 6.124c1.344.998 2.912 1.479 4.467 1.479 2.292 0 4.554-1.046 6.026-3.027 2.471-3.324 1.778-8.022-1.547-10.493l-4.613-3.427 4.613-3.427c3.325-2.471 4.018-7.169 1.547-10.493-2.471-3.325-7.168-4.017-10.493-1.548l-8.242 6.124-8.242-6.124c-3.325-2.469-8.023-1.778-10.493 1.548-2.471 3.324-1.778 8.022 1.547 10.493l4.613 3.427-4.613 3.427c-3.325 2.471-4.018 7.169-1.547 10.493z"/><path d="m191.937 181.419c.531 0 1.07-.057 1.61-.175l27.044-5.915c4.046-.886 6.609-4.883 5.724-8.93-.885-4.046-4.882-6.607-8.929-5.725l-27.044 5.915c-4.046.886-6.609 4.883-5.724 8.93.767 3.507 3.871 5.9 7.319 5.9z"/><path d="m326.45 199.438c-2.472-3.323-7.167-4.018-10.494-1.546l-8.241 6.123-8.241-6.123c-3.327-2.472-8.022-1.777-10.494 1.546-2.47 3.325-1.777 8.023 1.547 10.494l4.613 3.427-4.613 3.427c-3.324 2.471-4.017 7.169-1.547 10.494 1.473 1.98 3.734 3.026 6.026 3.026 1.555 0 3.123-.481 4.468-1.48l8.241-6.123 8.241 6.123c1.345.999 2.913 1.48 4.468 1.48 2.292 0 4.554-1.046 6.026-3.026 2.47-3.325 1.777-8.023-1.547-10.494l-4.613-3.427 4.613-3.427c3.324-2.471 4.017-7.169 1.547-10.494z"/><path d="m322.033 166.59-27.044-5.915c-4.05-.888-8.044 1.678-8.93 5.725-.885 4.047 1.678 8.044 5.725 8.93l27.044 5.915c.54.118 1.079.175 1.609.175 3.448 0 6.553-2.393 7.32-5.899.885-4.048-1.677-8.045-5.724-8.931z"/><path d="m170.384 471.731c-4.142 0-7.5 3.357-7.5 7.5v25.644c0 4.143 3.358 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-25.644c0-4.142-3.358-7.5-7.5-7.5z"/><path d="m340.991 471.731c-4.143 0-7.5 3.357-7.5 7.5v25.644c0 4.143 3.357 7.5 7.5 7.5s7.5-3.357 7.5-7.5v-25.644c0-4.142-3.357-7.5-7.5-7.5z"/><path d="m429.473 87.452 38.732-73.712c1.858-3.533 1.024-7.798-2.027-10.371-3.053-2.571-7.397-2.673-10.565-.243l-34.286 26.293-3.074-5.832c-2.317-4.394-6.659-7.211-11.615-7.537-4.945-.338-9.629 1.897-12.502 5.95l-40.159 56.658c-2.134 3.01-2.073 7.012.15 9.956 2.224 2.943 6.055 4.098 9.541 2.867l35.893-12.699 4.95 8.941c2.503 4.522 7.246 7.314 12.406 7.314.052 0 .104 0 .155-.001 5.221-.056 9.972-2.962 12.401-7.584zm-12.574-8.319-7.291-13.171c-1.353-2.443-3.898-3.869-6.563-3.869-.833 0-1.678.14-2.501.431l-22.351 7.907 27.41-38.671 6.599 12.518c1.031 1.954 2.871 3.355 5.029 3.828 2.159.472 4.416-.029 6.17-1.375l17.627-13.518z"/><path d="m107.864 87.724 4.95-8.941 35.889 12.696c3.48 1.237 7.314.086 9.54-2.858 2.228-2.943 2.291-6.946.155-9.963l-40.159-56.658c-2.872-4.053-7.547-6.28-12.502-5.95-4.957.326-9.298 3.145-11.614 7.538l-3.074 5.831-34.288-26.293c-3.169-2.43-7.512-2.329-10.565.243-3.052 2.573-3.886 6.838-2.029 10.367l19.537 37.182c1.927 3.668 6.461 5.08 10.128 3.151 3.667-1.927 5.077-6.462 3.15-10.128l-5.637-10.729 17.628 13.518c1.753 1.346 4.009 1.845 6.169 1.375 2.159-.473 3.999-1.874 5.029-3.829l6.599-12.517 27.41 38.671-22.352-7.907c-3.457-1.223-7.289.231-9.063 3.438l-7.292 13.172-3.624-6.896c-1.927-3.667-6.46-5.075-10.128-3.149-3.667 1.927-5.077 6.461-3.15 10.128l4.329 8.236c2.428 4.622 7.179 7.528 12.399 7.585.052.001.104.001.157.001 5.161 0 9.905-2.792 12.408-7.314z"/><path d="m73.273 134.536c-1.151-2.043-3.192-3.431-5.517-3.749-2.323-.319-4.663.47-6.321 2.127l-4.072 4.072-7.67-16.588c-1.739-3.761-6.196-5.398-9.956-3.66-3.759 1.738-5.398 6.196-3.66 9.955l12.182 26.344c1.029 2.226 3.08 3.807 5.494 4.236 2.412.431 4.884-.348 6.617-2.08l4.685-4.685 9.551 16.953c1.377 2.445 3.921 3.82 6.541 3.819 1.247 0 2.511-.312 3.674-.967 3.609-2.033 4.886-6.606 2.853-10.216z"/><path d="m472.638 116.738c-3.761-1.736-8.218-.101-9.956 3.659l-7.671 16.589-4.072-4.072c-1.658-1.658-3.995-2.45-6.322-2.127-2.323.318-4.364 1.706-5.516 3.75l-14.4 25.563c-2.033 3.608-.756 8.182 2.854 10.215 1.163.656 2.427.967 3.674.967 2.62 0 5.164-1.375 6.541-3.82l9.551-16.952 4.684 4.685c1.733 1.732 4.199 2.513 6.618 2.08 2.413-.43 4.464-2.011 5.493-4.235l12.183-26.344c1.737-3.761.098-8.218-3.661-9.958z"/></g></svg>
                    </div>
                    <h3 class="info__title">Strong Headache</h3>
                    <p class="info__text">
                        For people who have migraine attacks, a COVID-19 headache has also been described as moderate to severe.
                    </p>
                </div><!-- end info-box -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 column-td-6">
                <div class="info-box">
                    <div class="info__icon mx-auto">
                        <svg class="info_svg" id="_x33_0" enable-background="new 0 0 64 64" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><g><path d="m31 22h2v4h-2z"></path><path d="m39.555 23.168-1.752-1.168 1.752-1.168-1.11-1.664-4.248 2.832 4.248 2.832z"></path><path d="m25.555 24.832 4.248-2.832-4.248-2.832-1.11 1.664 1.752 1.168-1.752 1.168z"></path><path d="m54 31h-.879c-2.698 0-5.039 1.776-5.832 4.272-.01-.002-.02-.005-.03-.007l-6.488-1.145c-.124-.339-.273-.665-.433-.983 1.587-1.77 2.688-3.867 3.24-6.137h.422c2.206 0 4-1.794 4-4 0-.027-.007-.052-.008-.079 3.391-.488 6.008-3.424 6.008-6.971 0-.192.072-.365.207-.5 1.156-1.156 1.793-2.693 1.793-4.329 0-3.375-2.746-6.121-6.121-6.121h-.879c-1.542 0-3.023.409-4.354 1.15-2.636-3.144-6.588-5.15-11.002-5.15h-3.288c-3.563 0-6.82 1.313-9.333 3.47-1.237-2.115-3.498-3.47-6.023-3.47-3.86 0-7 3.163-7 7.05 0 .192-.072.365-.207.5-1.156 1.156-1.793 2.693-1.793 4.329 0 3.375 2.746 6.121 6.121 6.121h.879c1.149 0 2.28-.23 3.337-.654.035.163.051.33.091.492l.429 1.717c-.528.678-.857 1.52-.857 2.445 0 2.206 1.794 4 4 4h.422c.552 2.27 1.653 4.367 3.24 6.137-.16.318-.309.644-.433.983l-6.274 1.107c-.386-3.497-3.356-6.227-6.955-6.227-3.86 0-7 3.163-7 7.05 0 .192-.072.365-.207.5-1.156 1.156-1.793 2.693-1.793 4.329 0 3.01 2.186 5.513 5.052 6.021-.034.385-.052.775-.052 1.168v7.568c0 4.061 3.303 7.364 7.364 7.364h44.636v-14.296c2.941-.871 5-3.563 5-6.704v-2c0-4.962-4.038-9-9-9zm-19.664 14.078.646 5.813-.024.072c-.406 1.218-1.542 2.037-2.828 2.037-.999 0-1.926-.496-2.48-1.328-.254-.381-.583-.697-.949-.959zm2.368 3.197-.557-5.008.525-.525c.856-.856 1.328-1.995 1.328-3.206 0-2.501-2.035-4.536-4.536-4.536-1.211 0-2.351.472-3.207 1.329l-.671.671h-.172c-1.179 0-2.288.459-3.121 1.293-.834.833-1.293 1.942-1.293 3.121v.172l-1.464 1.464 1.232-6.982c.621-3.516 3.662-6.068 7.232-6.068s6.611 2.552 7.231 6.067l1.756 9.947c.009.049.013.098.013.147 0 .463-.376.839-.839.839-1.302 0-2.51.478-3.457 1.275zm7.296-23.275h-.068c.045-.464.068-.932.068-1.403v-2.597c1.103 0 2 .897 2 2s-.897 2-2 2zm3.443-13.56c.466-.285 1.002-.44 1.557-.44h.879c.065 0 .121.056.086.207-1.267 1.267-1.965 2.951-1.965 4.743 0 .085-.032.158-.051.236.016-.277.051-.553.051-.83 0-1.359-.202-2.669-.557-3.916zm1.557-4.44h.879c2.272 0 4.121 1.849 4.121 4.121 0 1.101-.429 2.136-1.208 2.915-.511.511-.792 1.191-.792 1.914 0 2.639-2.022 4.788-4.58 5.007-.084-.141-.177-.274-.278-.403l.403-1.611c1.394-.261 2.455-1.501 2.455-2.993 0-1.257.49-2.439 1.38-3.33.4-.4.62-.933.62-1.499 0-1.169-.952-2.121-2.121-2.121h-.879c-.788 0-1.546.204-2.236.552-.274-.617-.595-1.208-.95-1.776.982-.505 2.065-.776 3.186-.776zm-36 10h-.879c-2.272 0-4.121-1.849-4.121-4.121 0-1.101.429-2.136 1.208-2.915.511-.511.792-1.191.792-1.914 0-2.784 2.243-5.05 5-5.05 1.983 0 3.739 1.167 4.543 2.943-.578.663-1.093 1.379-1.543 2.14v-.083c0-1.654-1.346-3-3-3s-3 1.368-3 3.05c0 1.257-.49 2.439-1.38 3.33-.4.4-.62.933-.62 1.499 0 1.169.952 2.121 2.121 2.121h.879c1.164 0 2.222-.416 3.073-1.086-.047.474-.073.955-.073 1.442 0 .309.037.616.057.924-.95.464-1.992.72-3.057.72zm3-9v2c0 1.654-1.346 3-3 3h-.879c-.065 0-.121-.056-.086-.207 1.267-1.267 1.965-2.951 1.965-4.743 0-.589.439-1.05 1-1.05.551 0 1 .449 1 1zm4 17c-1.103 0-2-.897-2-2s.897-2 2-2v2.597c0 .472.023.94.068 1.403zm.162-6h-.162c-.495 0-.964.102-1.402.267l-.229-.915c-.245-.978-.369-1.986-.369-2.996 0-6.813 5.543-12.356 12.356-12.356h3.288c6.813 0 12.356 5.543 12.356 12.356 0 1.01-.124 2.018-.369 2.997l-.229.915c-.438-.166-.907-.268-1.402-.268h-.274c-.411-1.365-1.274-2.567-2.494-3.38-.536-.357-1.122-.628-1.74-.805l-4.736-1.353c.144-.466.244-.95.244-1.462v-5h-2v5c0 1.654-1.346 3-3 3h-4.697c-1.05 0-2.067.308-2.941.891-1.109.739-1.882 1.85-2.2 3.109zm1.838 4.597v-3.294c0-1.107.55-2.134 1.471-2.748.544-.363 1.178-.555 1.832-.555h4.697c1.507 0 2.844-.684 3.762-1.742l5.181 1.48c.419.12.816.303 1.18.546 1.175.783 1.877 2.094 1.877 3.508v2.805c0 2.88-.983 5.61-2.788 7.807-1.726-2.096-4.332-3.404-7.212-3.404s-5.486 1.308-7.212 3.403c-1.805-2.196-2.788-4.926-2.788-7.806zm.706 12.647-1.636 9.272-5.07 5.07v-3.586h-2v4h-6v-2.932c0-5.348 3.822-9.903 9.088-10.833zm-14.706 4.756h-.879c-.065 0-.121-.056-.086-.207 1.267-1.267 1.965-2.951 1.965-4.743 0-.589.439-1.05 1-1.05.551 0 1 .449 1 1v1.829c-1.137.888-2.115 1.956-2.898 3.162-.034.002-.067.009-.102.009zm-5-.121c0-1.101.429-2.136 1.208-2.915.511-.511.792-1.191.792-1.914 0-2.784 2.243-5.05 5-5.05 2.661 0 4.823 2.095 4.972 4.719-.686.223-1.344.499-1.972.825v-.544c0-1.654-1.346-3-3-3s-3 1.368-3 3.05c0 1.257-.49 2.439-1.38 3.33-.4.4-.62.933-.62 1.499 0 1.141.909 2.069 2.04 2.113-.264.623-.482 1.268-.648 1.935-1.925-.347-3.392-2.025-3.392-4.048zm5 14.757v-2.636h5.586l-2.293 2.293 1.414 1.414 14.293-14.293v-1c0-.635.258-1.258.707-1.707s1.072-.707 1.707-.707h1l1.257-1.257c.479-.479 1.116-.743 1.793-.743 1.398 0 2.536 1.138 2.536 2.536 0 .677-.264 1.314-.743 1.793l-18.1 18.1c-1.013 1.013-2.36 1.571-3.793 1.571-2.958 0-5.364-2.406-5.364-5.364zm10.571 5.207 8.678-8.678c.295.135.554.343.737.617.926 1.389 2.475 2.218 4.144 2.218 2.148 0 4.047-1.369 4.725-3.405l.094-.28c.462-1.385 1.752-2.315 3.212-2.315 1.566 0 2.839-1.273 2.839-2.839 0-.166-.014-.331-.043-.494l-1.663-9.424 5.618.991c5.266.931 9.088 5.486 9.088 10.834v2.932h-6v-4h-2v14h-29.602c.057-.053.117-.101.173-.157zm38.429-18.843c0 .221-.082.423-.208.591-.625-1.348-1.476-2.573-2.509-3.626-.322-.646-.734-1.249-1.283-1.844 0-.065.056-.121.121-.121h.879c1.654 0 3 1.346 3 3zm-7 19v-8h6v8zm11-19c0 2.066-1.253 3.851-3.091 4.608-.078-.694-.203-1.375-.386-2.034.886-.528 1.477-1.493 1.477-2.574v-2c0-2.757-2.243-5-5-5h-.879c-.983 0-1.805.676-2.044 1.585-.59-.312-1.206-.581-1.849-.802.562-1.631 2.11-2.783 3.893-2.783h.879c3.86 0 7 3.14 7 7z"></path><path d="m57 23c0-1.103.897-2 2-2s2 .897 2 2-.897 2-2 2v2c2.206 0 4-1.794 4-4s-1.794-4-4-4-4 1.794-4 4z"></path><path d="m4 25c1.654 0 3-1.346 3-3s-1.346-3-3-3-3 1.346-3 3 1.346 3 3 3zm0-4c.551 0 1 .449 1 1s-.449 1-1 1-1-.449-1-1 .449-1 1-1z"></path></g></svg>
                    </div>
                    <h3 class="info__title">Runny Nose</h3>
                    <p class="info__text">
                    A runny nose is a discharge of mucus from the nostrils
                    </p>
                </div><!-- end info-box -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 column-td-6">
                <div class="info-box">
                    <div class="info__icon mx-auto">
                        <svg class="info_svg" xmlns="http://www.w3.org/2000/svg" id="outline" viewBox="0 0 512 512"><path d="M248,480H72V320h56a7.99943,7.99943,0,0,0,6.65625-3.5625l48-72A7.99977,7.99977,0,0,0,176,232h-8a32,32,0,0,1,0-64h32a8.00039,8.00039,0,0,0,8-8V104H312v80a8.00115,8.00115,0,0,0,1.59961,4.7998l19.20019,25.59961A16.00024,16.00024,0,0,1,320,240H296v16h16v32H294.627a62.21991,62.21991,0,0,0-44.28418,18.34277l11.31446,11.31446A46.3218,46.3218,0,0,1,294.627,304H312v32h16V254.9873a32.004,32.004,0,0,0,17.60059-50.1875L328,181.333V56a40.04584,40.04584,0,0,0-40-40H147.89453C75.168,16,16,75.168,16,147.89453V168a8.01629,8.01629,0,0,0,.292,2.1416L56,313.09033V488a8.00039,8.00039,0,0,0,8,8H256a8.00039,8.00039,0,0,0,8-8V448H248ZM32,147.89453A116.026,116.026,0,0,1,147.89453,32H288a24.0275,24.0275,0,0,1,24,24V88H200a8.00039,8.00039,0,0,0-8,8v56H168a48.00205,48.00205,0,0,0-6.64258,95.542L123.71875,304H70.08105L32,166.90918Z"/><circle cx="256" cy="176" r="16"/><path d="M424,328a71.58279,71.58279,0,0,0-28.40039,5.84863L209.70215,373.2998l3.39746-16.98535a7.9997,7.9997,0,0,0-9.41406-9.41406l-23.65039,4.73047L166.65625,331.5625a7.99959,7.99959,0,0,0-13.3125,0l-13.37891,20.06836-23.65039-4.73047a7.9997,7.9997,0,0,0-9.41406,9.41406l4.73047,23.65039L91.5625,393.34375a7.99959,7.99959,0,0,0,0,13.3125l20.06836,13.37891-4.73047,23.65039a7.9997,7.9997,0,0,0,9.41406,9.41406l23.65039-4.73047,13.37891,20.06836a7.99959,7.99959,0,0,0,13.3125,0l13.37891-20.06836,23.65039,4.73047a7.9997,7.9997,0,0,0,9.41406-9.41406l-3.4707-17.35157L394.145,465.49268A71.98987,71.98987,0,1,0,424,328ZM228.4375,393.34375l-8.71826-5.812,147.10254-31.21826a71.81453,71.81453,0,0,0-.33057,86.95312l-146.37305-31.064,8.31934-5.54639a7.99959,7.99959,0,0,0,0-13.3125Zm-50.51758,38.28516a8.00173,8.00173,0,0,0-8.22558,3.40722L160,449.57715l-9.69434-14.541a8.00087,8.00087,0,0,0-8.22558-3.40722l-17.13672,3.42773,3.42773-17.13672a7.99978,7.99978,0,0,0-3.40722-8.22558L110.42285,400l14.541-9.69434a7.99978,7.99978,0,0,0,3.40722-8.22558l-3.42773-17.13672,17.13672,3.42773a7.99634,7.99634,0,0,0,8.22558-3.40722L160,350.42285l9.69434,14.541a7.9987,7.9987,0,0,0,8.22558,3.40722l17.13672-3.42773-3.42773,17.13672a7.99978,7.99978,0,0,0,3.40722,8.22558L209.57715,400l-14.541,9.69434a7.99978,7.99978,0,0,0-3.40722,8.22558l3.42773,17.13672Zm288.99024,4.30273-3.75489-7.50976a8.00005,8.00005,0,0,0-14.31054,7.15624l5.67041,11.34131a55.83745,55.83745,0,0,1-59.2461,1.126l5.76123-5.76123a8.00052,8.00052,0,0,0-11.31445-11.31445l-6.91309,6.91308a55.964,55.964,0,0,1,77.12891-80.793l-7.50976,3.75489a8.00005,8.00005,0,1,0,7.15624,14.31054l11.34131-5.67041a55.83956,55.83956,0,0,1-4.00927,66.44678Z"/><path d="M413.65723,402.34277a8.003,8.003,0,0,0-11.31446,0l-8,8a8.00053,8.00053,0,0,0,11.31446,11.31446l8-8A8.00122,8.00122,0,0,0,413.65723,402.34277Z"/><path d="M418.34277,389.65723a8.00053,8.00053,0,0,0,11.31446-11.31446l-8-8a8.00053,8.00053,0,0,0-11.31446,11.31446Z"/><path d="M395.57715,399.15527a7.99944,7.99944,0,0,0,3.57812-10.73242l-4.62695-9.25488a7.99962,7.99962,0,1,0-14.31055,7.1543l4.627,9.25488a7.99279,7.99279,0,0,0,10.73242,3.57812Z"/><path d="M437.7793,404.167l-9.35645,4.67774a7.99962,7.99962,0,1,0,7.1543,14.31054l9.35644-4.67773A7.99962,7.99962,0,0,0,437.7793,404.167Z"/><path d="M432,432h-8a8,8,0,0,0,0,16h8a8,8,0,0,0,0-16Z"/><path d="M469.65723,394.34277l-8-8a8.00053,8.00053,0,0,0-11.31446,11.31446l8,8a8.00053,8.00053,0,0,0,11.31446-11.31446Z"/></svg>
                    </div>
                    <h3 class="info__title">Sore Throat</h3>
                    <p class="info__text">
                    A sore throat is pain, scratchiness or irritation of the throat that often worsens when you swallow.
                    </p>
                </div><!-- end info-box -->
            </div><!-- end col-lg-4 -->
            <div class="col-lg-4 column-td-6">
                <div class="info-box">
                    <div class="info__icon mx-auto">
                        <svg class="info_svg" id="Layer_1" enable-background="new 0 0 512.019 512.019" viewBox="0 0 512.019 512.019" xmlns="http://www.w3.org/2000/svg"><g><g><path d="m466.009 299.776v-245.157c0-16.542-13.458-30-30-30h-12.059c-.012-2.615-.32-5.137-.95-7.505-1.745-6.561-5.837-11.755-11.524-14.626-14.005-7.07-33.673 1.374-45.811 19.646-26.403-6.864-66.285-10.888-109.113-10.888-43.444 0-83.649 4.113-110.039 11.133-.021-.033-.043-.065-.064-.099-12.131-18.371-31.866-26.88-45.908-19.792-5.687 2.871-9.779 8.065-11.523 14.626-.629 2.368-.938 4.89-.95 7.505h-12.059c-16.542 0-30 13.458-30 30v245.157c-11.639 4.128-20 15.243-20 28.28v80c0 13.036 8.361 24.152 20 28.28v65.683c0 5.522 4.477 10 10 10h400c5.523 0 10-4.478 10-10v-65.683c11.639-4.128 20-15.243 20-28.28v-80c0-13.037-8.361-24.152-20-28.28zm-20-245.157v243.437h-58.242c-7.6-19.003-19.393-36.234-34.192-50.358 4.298-.863 8.34-1.782 12.092-2.759 9.248 13.915 22.862 22.131 35.015 22.13 3.8 0 7.459-.804 10.794-2.487 5.687-2.871 9.78-8.064 11.524-14.626 2.595-9.758-.163-22.106-7.376-33.031-3.269-4.949-7.31-9.387-11.752-12.96 4.641-18.974 7.258-44.121 7.258-70.43 0-26.308-2.617-51.456-7.258-70.429 4.442-3.573 8.484-8.012 11.752-12.961 1.194-1.808 2.254-3.657 3.199-5.525h17.186c5.515-.001 10 4.486 10 9.999zm-255.286 105.862c11.046.179 21.198-.814 26.14-1.406 12.179-1.459 39.076-5.874 56.012-17.588 9.368 8.987 27.146 22.896 48.438 24.19v15.138c0 36.004-29.292 65.295-65.295 65.295s-65.295-29.291-65.295-65.295zm130.59-14.991c-21.663-2.14-39.791-23.89-40.003-24.15-1.878-2.312-4.688-3.666-7.667-3.694-3.012-.034-5.814 1.272-7.735 3.548-6.666 7.895-27.331 15.137-51.424 18.023-8.955 1.073-16.923 1.411-23.693 1.267 1.413-34.764 30.122-62.614 65.227-62.614 36.004 0 65.295 29.292 65.295 65.296zm-123.288 97.793c15.231 14.15 35.614 22.825 57.993 22.825 22.38 0 42.762-8.676 57.993-22.825 22.501 12.316 40.859 31.75 51.925 54.773h-219.837c11.067-23.024 29.421-42.454 51.926-54.773zm-89.679-221.03c.399-1.503.987-1.799 1.208-1.911 3.079-1.553 13.091 2.184 20.206 12.96 1.084 1.642 2.018 3.33 2.776 5.021 2.094 4.665 7.372 6.996 12.231 5.41 23.557-7.702 66.391-12.486 111.786-12.486 45.385 0 86.851 4.563 110.92 12.205 4.813 1.529 10.012-.772 12.117-5.364.741-1.617 1.639-3.227 2.669-4.786 7.114-10.774 17.124-14.51 20.205-12.959.221.112.809.408 1.208 1.911.915 3.44-.195 9.992-4.737 16.87-3.109 4.708-7.18 8.628-11.463 11.037-4.074 2.291-6.017 7.098-4.68 11.576 5.299 17.751 8.338 43.92 8.338 71.798s-3.039 54.048-8.338 71.799c-1.337 4.479.606 9.285 4.68 11.576 4.283 2.409 8.354 6.328 11.462 11.035 4.542 6.88 5.653 13.432 4.738 16.872-.399 1.503-.987 1.799-1.208 1.91-3.084 1.556-13.092-2.184-20.205-12.958-1.031-1.562-1.929-3.172-2.669-4.786-2.104-4.592-7.302-6.892-12.116-5.364-9.008 2.86-21.074 5.396-35.046 7.378-1.725-1.116-3.474-2.195-5.248-3.235 8.92-13.476 14.132-29.612 14.132-46.948v-17.872c1.406-.482 2.817-1.011 4.237-1.63 5.062-2.21 7.373-8.105 5.163-13.166-1.677-3.842-5.477-6.094-9.426-5.992-.548-46.565-38.577-84.282-85.27-84.282-45.299 0-82.448 35.501-85.125 80.14-2.094-.627-3.58-1.318-4.396-2.007-4.221-3.563-10.53-3.027-14.092 1.193-3.562 4.222-3.027 10.53 1.194 14.092 4.238 3.576 10.33 5.846 17.123 7.242v22.284c0 17.336 5.212 33.471 14.132 46.948-1.706 1-3.389 2.038-5.05 3.109-14.058-2.028-26.12-4.618-35.037-7.533-4.861-1.587-10.137.745-12.231 5.41-.757 1.688-1.692 3.378-2.776 5.021-7.114 10.775-17.125 14.516-20.206 12.96-.221-.112-.809-.408-1.208-1.911-.915-3.441.195-9.992 4.738-16.871 3.108-4.708 7.18-8.627 11.464-11.037 4.073-2.292 6.016-7.099 4.679-11.576-5.298-17.75-8.337-43.919-8.337-71.798 0-27.878 3.039-54.047 8.337-71.797 1.337-4.478-.606-9.284-4.679-11.576-4.284-2.41-8.355-6.329-11.464-11.037-4.54-6.883-5.651-13.434-4.736-16.875zm-32.337 22.366h17.185c.946 1.868 2.006 3.718 3.2 5.526 3.268 4.948 7.31 9.387 11.752 12.96-4.641 18.973-7.258 44.12-7.258 70.429s2.617 51.457 7.258 70.43c-4.443 3.573-8.484 8.012-11.752 12.96-7.214 10.925-9.971 23.272-7.377 33.031 1.744 6.561 5.836 11.755 11.524 14.627 14.042 7.086 33.777-1.423 45.909-19.795.021-.031.042-.063.063-.095 3.757 1 7.813 1.941 12.135 2.825-14.887 14.156-26.747 31.451-34.38 50.539h-58.26v-243.437c.001-5.513 4.487-10 10.001-10zm390 363.437c0 5.514-4.486 10-10 10h-155.003c-5.523 0-10 4.478-10 10s4.477 10 10 10h145.003v53.963h-380v-53.963h144.997c5.523 0 10-4.478 10-10s-4.477-10-10-10h-154.997c-5.514 0-10-4.486-10-10v-80c0-5.514 4.486-10 10-10h400c5.514 0 10 4.486 10 10z"/><path d="m243.088 196.609c3.906-3.904 3.906-10.236.001-14.142s-10.237-3.905-14.143-.001c-1.275 1.274-2.97 1.977-4.773 1.977s-3.498-.702-4.774-1.978c-3.905-3.904-10.237-3.903-14.142.002-3.905 3.906-3.904 10.238.002 14.143 5.053 5.051 11.77 7.833 18.914 7.833 7.145 0 13.863-2.783 18.915-7.834z"/><path d="m287.862 204.443c7.144 0 13.861-2.782 18.914-7.833 3.906-3.904 3.907-10.236.002-14.143-3.904-3.905-10.236-3.906-14.142-.002-1.276 1.275-2.971 1.978-4.774 1.978s-3.498-.702-4.773-1.977c-3.906-3.906-10.237-3.905-14.143.001-3.905 3.905-3.904 10.237.001 14.142 5.053 5.051 11.77 7.834 18.915 7.834z"/><path d="m256.009 418.056h-.007c-5.523 0-9.996 4.478-9.996 10s4.48 10 10.003 10 10-4.478 10-10-4.477-10-10-10z"/></g></g></svg>
                    </div>
                    <h3 class="info__title">Tiredness</h3>
                    <p class="info__text">
                    Tiredness or exhaustion that goes on for a long time
                        Risu viverra maenas.
                    </p>
                </div><!-- end info-box -->
            </div><!-- end col-lg-4 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end service-area -->
<!--======================================
        END SERVICE AREA
======================================-->

<div class="section-block"></div>

<!--======================================
        START SERVICE AREA
======================================-->
<section class="service-area section--padding">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="section-heading text-center">
                    <h5 class="section__meta">How to be Safe</h5>
                    <h2 class="section__title padding-bottom-20px line-height-55">Important Precautions <br> Follow one by one</h2>
                    <div class="section-divider"></div>
                </div><!-- end section-heading -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
        <div class="row padding-top-100px">
            <div class="col-lg-6">
                <div class="note-box do-not-box">
                    <div class="note-box-header">
                        <div class="icon-element">
                            <i class="la la-times"></i>
                        </div>
                        <h3 class="widget-title note-title text-uppercase"> Things you should not do</h3>
                    </div>
                    <div class="section-block"></div>
                    <ul class="list-items">
                        <li class="d-flex align-items-center mb-4">
                            <div class="about-icon flex-shrink-0">
                                <svg viewBox="0 0 487 487.95208" xmlns="http://www.w3.org/2000/svg"><path d="m244.453125 0c-98.679687 0-187.644531 59.441406-225.40625 150.609375s-16.886719 196.105469 52.886719 265.882813c95.28125 95.277343 249.757812 95.28125 345.035156 0 95.277344-95.277344 95.277344-249.753907 0-345.03125-45.652344-45.90625-107.773438-71.636719-172.515625-71.460938zm227.976563 243.976562c-.003907 92.207032-55.546876 175.332032-140.734376 210.617188s-183.242187 15.78125-248.441406-49.417969c-58.144531-57.476562-81.058594-141.695312-60.050781-220.707031 21.011719-79.015625 82.726563-140.730469 161.738281-161.738281 79.015625-21.011719 163.230469 1.902343 220.710938 60.046875 42.894531 42.65625 66.941406 100.703125 66.777344 161.199218zm0 0"/><path d="m398.496094 107.507812c-2.242188-.101562-4.421875.746094-6.007813 2.335938l-282.171875 282.167969c-1.585937 1.585937-2.433594 3.765625-2.335937 6.007812.097656 2.238281 1.132812 4.335938 2.851562 5.777344 82.757813 69.191406 204.648438 63.761719 280.925781-12.515625 76.277344-76.277344 81.707032-198.164062 12.515626-280.925781-1.441407-1.71875-3.539063-2.75-5.777344-2.847657zm-92.367188 159.753907c0 3.378906 2.125 6.390625 5.304688 7.527343 3.179687 1.136719 6.734375.15625 8.875-2.453124l16.671875-20.300782c3.839843-4.816406 9.503906-7.824218 15.644531-8.300781 5.992188-.386719 11.875 1.730469 16.246094 5.847656.101562.09375.199218.183594.308594.269531l1.25 1.039063-56.59375 108.566406c-8.566407 16.460938-24.722657 26.519531-42.160157 26.519531h-66.824219c-16.820312 0-32.492187-9.476562-41.230468-24.808593l142.472656-142.390625zm74.3125 112.703125c-68.054687 68.035156-175.914062 75.320312-252.5 17.046875l24.058594-24.222657c11.914062 18.125 31.660156 29.1875 52.851562 29.1875h66.820313c23.425781 0 45.019531-13.359374 56.351563-35.132812l59.578124-114.375c1.742188-3.328125.910157-7.421875-1.988281-9.808594l-6.078125-5.023437c-7.582031-7.015625-17.726562-10.585938-28.035156-9.871094-10.53125.769531-20.269531 5.878906-26.882812 14.109375l-2.503907 3.054688-.03125-42.058594 75.410157-75.40625c58.269531 76.582031 50.988281 184.445312-17.050782 252.5zm0 0"/><path d="m380.921875 89.933594c-.097656-2.238282-1.132813-4.335938-2.847656-5.777344-82.761719-69.1875-204.648438-63.757812-280.921875 12.519531-76.277344 76.277344-81.710938 198.164063-12.519532 280.921875 1.441407 1.71875 3.535157 2.753906 5.777344 2.851563.117188 0 .234375.007812.351563.007812 2.121093 0 4.15625-.84375 5.65625-2.34375l282.167969-282.171875c1.585937-1.585937 2.433593-3.765625 2.335937-6.007812zm-224.480469 73.882812c.019532-7.71875 5.660156-13.839844 12.578125-13.839844h.457031c6.933594 0 12.976563 5.980469 12.976563 13.722657v71.277343h16v-121.222656c0-7.742187 5.542969-14.039062 12.523437-14.039062 6.933594 0 12.476563 6.296875 12.476563 14.039062v114.898438l-66.921875 66.902344c-.035156-46.617188-.164062-99.726563-.089844-131.738282zm108.441406-36.828125c-.003906.273438-.007812-.011719-.007812.988281h.007812c0 10-.003906 19.917969-.007812 28.601563 0 14.023437.09375 25.042969.09375 30.535156l-25.515625 25.535157v-116.960938c0-7.742188 5.441406-13.710938 12.375-13.710938h.484375c6.933594 0 12.574219 6.214844 12.574219 13.953126 0 10.414062 0 20.917968-.003907 31.058593zm15.988282 44.042969c0-4.347656 0-9.265625 0-14.617188 0-8.847656.007812-18.859374.007812-29.351562.273438-7.476562 5.800782-13.464844 12.574219-13.464844 6.925781 0 12.566406 6.292969 12.574219 14.027344l.011718 18.234375zm41.160156-41.164062v-2.253907c-.011719-16.550781-13.046875-30.015625-28.800781-30.015625-4.4375.023438-8.8125 1.070313-12.777344 3.058594v-4.980469c0-16.558593-12.386719-29.699219-28.140625-29.699219h-.484375c-13.023437 0-24.039063 9.039063-27.46875 21.582032-4.09375-2.4375-8.765625-3.734375-13.53125-3.765625-15.757813 0-28.375 13.394531-28.375 29.960937v22.972656c-4.058594-1.894531-8.5-2.835937-12.976563-2.75h-.457031c-15.71875 0-28.535156 13.28125-28.570312 29.800782-.085938 35.589844 0 97.15625 0 146.957031v.71875l-49.035157 49.03125c-44.273437-58.164063-51.75-136.394531-19.292968-201.890625 32.457031-65.496094 99.234375-106.933594 172.332031-106.933594 42.125-.226562 83.121094 13.59375 116.511719 39.277344zm0 0"/></svg>
                            </div>
                            <div class="about-list-item pl-3">
                                <h3 class="widget-title pb-2">Do Not Share Eating</h3>
                                <p>
                                    Food is a way of transmission of COVID-19
                                </p>
                            </div>
                        </li>
                        <li class="d-flex align-items-center mb-4">
                            <div class="about-icon flex-shrink-0">
                                <svg version="1.1" id="Capa_4" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                     viewBox="0 0 358.4 358.4" style="enable-background:new 0 0 358.4 358.4;" xml:space="preserve">
<g>
	<g>
		<path d="M333.473,165.664c-0.64-0.64-1.328-1.2-2.064-1.632c-10.368-6.528-22.192-9.824-34.016-9.824
			c-16.368,0-32.72,6.24-45.2,18.72l-11.328,11.312V172.8l90.144-90.144l-33.76-33.76l-56.384,56.384V32c0-17.6-14.4-32-32-32
			c-17.6,0-32,14.4-32,32v70.4c-5.84-7.168-14.64-11.84-24.608-11.84c-6.928,0-13.312,2.272-18.544,6.032L64.801,136.48
			c-9.52,5.552-15.936,15.792-15.936,27.568v52.624c0,22.72,7.648,43.472,20.064,60.544L21.521,324.64l33.76,33.76l49.904-49.904
			c14.208,7.36,30.16,11.76,47.264,11.76c27.712,0,52.816-10.944,71.392-28.672l0.016,0.144l1.776-1.776
			c0.032-0.032,0.064-0.064,0.08-0.08l107.744-107.76C338.017,177.568,338.017,170.208,333.473,165.664z M64.865,216.672v-52.624
			c0-5.632,2.992-10.736,8.016-13.664l70.144-40.784c1.92-1.376,5.088-3.04,9.216-3.04c6.4,0,10.4,3.728,12.224,5.952l14.208,17.408
			c2.144,2.624,5.696,3.616,8.88,2.48c3.184-1.136,5.312-4.144,5.312-7.536V32c0-8.816,7.184-16,16-16c8.816,0,16,7.184,16,16
			v89.296L80.353,265.808C70.753,251.744,64.865,234.944,64.865,216.672z M55.281,335.776L44.145,324.64l253.12-253.12
			l11.136,11.136L55.281,335.776z M212.801,280c-16.4,15.632-37.84,24.24-60.352,24.24c-12.608,0-24.528-2.816-35.376-7.648
			L224.865,188.8v3.136c0,6.352,3.744,11.76,9.12,14.304c3.056,1.44,6.688,0.816,9.072-1.584l20.432-20.432
			c9.056-9.056,21.088-14.032,33.888-14.032c7.216,0,14.272,1.6,20.704,4.672L212.801,280z"/>
	</g>
</g>
                                    <g>
</g>
                                    <g>
</g>
</svg>
                            </div>
                            <div class="about-list-item pl-3">
                                <h3 class="widget-title pb-2">Don’t Touch Your Face or Nose</h3>
                                <p>
                                    Your face is the entrance for the virus to your body, keep it safe
                                </p>
                            </div>
                        </li>
                        <li class="d-flex align-items-center mb-4">
                            <div class="about-icon flex-shrink-0">
                                <svg xmlns="http://www.w3.org/2000/svg" id="outline-2" viewBox="0 0 512 512"><path d="M160,136V240a40.04552,40.04552,0,0,0,40,40h8a8.00008,8.00008,0,0,0,8-8V224h48V208H216V136H200v80H176V136a32.03635,32.03635,0,0,1,32-32h26.86786a32.16391,32.16391,0,0,0,5.76117,4.05713l-14.28577,14.28564,11.31348,11.31446L248,123.314V192h16V123.314l10.34326,10.34327,11.31348-11.31446L271.371,108.05713A32.16391,32.16391,0,0,0,277.13214,104H304a32.03635,32.03635,0,0,1,32,32h16a48.054,48.054,0,0,0-48-48H286.98737A31.94765,31.94765,0,0,0,288,80V32a16.01833,16.01833,0,0,0-16-16H256a32.03635,32.03635,0,0,0-32,32V80a31.94765,31.94765,0,0,0,1.01263,8H208A48.054,48.054,0,0,0,160,136Zm40,96v32a24.0275,24.0275,0,0,1-24-24v-8ZM240,64h32V80a16,16,0,0,1-32,0Zm16-32h16V48H240A16.01833,16.01833,0,0,1,256,32Z"/><rect x="296" y="136" width="16" height="40"/><path d="M56,280V416H72V232h72V216H72V136H56v40H32V136a32.03635,32.03635,0,0,1,32-32H90.86786a31.91046,31.91046,0,0,0,42.26428,0H160V88H142.98737A31.94765,31.94765,0,0,0,144,80V48a32,32,0,0,0-64,0V80a31.94765,31.94765,0,0,0,1.01263,8H64a48.054,48.054,0,0,0-48,48V240A40.04552,40.04552,0,0,0,56,280ZM112,96A16.01833,16.01833,0,0,1,96,80V64h32V80A16.01833,16.01833,0,0,1,112,96Zm0-64a16.01833,16.01833,0,0,1,16,16H96A16.01833,16.01833,0,0,1,112,32ZM56,192v72a24.0275,24.0275,0,0,1-24-24V192Z"/><path d="M104,288v80h16V288a8,8,0,0,0-16,0Z"/><rect x="152" y="288" width="16" height="32"/><path d="M448,88H435.314l58.34278-58.34277L482.34326,18.34277l-464,464,11.31348,11.31446L59.314,464H104v16H56v16H160V480H120V464h40a8.00008,8.00008,0,0,0,8-8V355.314l32-32V452.686l-21.65674,21.65673A8.00076,8.00076,0,0,0,176,480v8a8.00008,8.00008,0,0,0,8,8H328a8.00008,8.00008,0,0,0,8-8v-8a8.00076,8.00076,0,0,0-2.34326-5.65723L312,452.686V280a40.04552,40.04552,0,0,0,40-40V171.314L419.314,104H448a32.03635,32.03635,0,0,1,32,32v40H456V136H440v72H368v16h72V448H408V288a8,8,0,0,0-16,0V448H360V288H344V456a8.00008,8.00008,0,0,0,8,8h40v16H352v16H472a8.00008,8.00008,0,0,0,8-8v-8a8.00076,8.00076,0,0,0-2.34326-5.65723L456,452.686V280a40.04552,40.04552,0,0,0,40-40V136A48.054,48.054,0,0,0,448,88ZM104,419.314V448H75.314ZM152,448H120V403.314l32-32ZM296,227.314V448H264V288a8,8,0,0,0-16,0V448H216V307.314ZM211.3139,464H248v16H195.31348ZM264,480V464h36.6861l16.00042,16Zm48-216V232h24v8A24.0275,24.0275,0,0,1,312,264Zm24-48H312v-4.686l24-24Zm72,264V464h36.6861l16.00042,16Zm48-216V192h24v48A24.0275,24.0275,0,0,1,456,264Z"/><path d="M352,104h30.11133a7.99962,7.99962,0,0,0,5.96094-13.335A15.97039,15.97039,0,0,1,384,80V64h8a7.99583,7.99583,0,0,0,3.57764-.84473L408,56.94434l12.42236,6.21093A7.99928,7.99928,0,0,0,432,56V48a32,32,0,0,0-64,0V80a32.02162,32.02162,0,0,0,1.01318,8H352Zm48-72a16.02827,16.02827,0,0,1,15.05664,10.584l-3.479-1.73925a7.99431,7.99431,0,0,0-7.15528,0L390.11133,48H384A16.01833,16.01833,0,0,1,400,32Z"/></svg>
                            </div>
                            <div class="about-list-item pl-3">
                                <h3 class="widget-title pb-2">Avoid Contact Sick People</h3>
                                <p>
                                    Avoid contact with sick people as much as you can as they may be carriers of the virus
                                </p>
                            </div>
                        </li>
                        <li class="d-flex align-items-center mb-4">
                            <div class="about-icon flex-shrink-0">
                                <svg id="Layer_3" enable-background="new 0 0 64 64" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><path d="m61 30c0-15.99-13.01-29-29-29s-29 13.01-29 29c0 3.658.674 7.218 1.998 10.59v3.41c0 1.654 1.346 3 3 3 .352 0 .686-.072 1-.184v9.184c0 1.654 1.346 3 3 3s3-1.346 3-3v-4c0-.552.448-1 1-1s1 .448 1 1v6c0 1.654 1.346 3 3 3s3-1.346 3-3v-4c0-.552.448-1 1-1s1 .448 1 1h.002v6c0 1.654 1.346 3 3 3s3-1.346 3-3v-4c0-.552.448-1 1-1s1 .448 1 1v4c0 1.654 1.346 3 3 3s3-1.346 3-3v-6c0-.552.448-1 1-1s1 .448 1 1v4c0 1.654 1.346 3 3 3s3-1.346 3-3v-6c0-.552.448-1 1-1s1 .448 1 1v4c0 1.654 1.346 3 3 3s3-1.346 3-3v-9.184c.314.112.648.184 1 .184 1.654 0 3-1.346 3-3v-3.406c1.325-3.375 2-6.936 2-10.594zm-2.428 4.787c-.883-2.215-3.045-3.787-5.572-3.787h-.542c.742-.827 1.257-1.858 1.45-3h5.01c.049.661.082 1.327.082 2 0 1.62-.147 3.218-.428 4.787zm-46.572-7.787c0-2.206 1.794-4 4-4 1.458 0 2.795.804 3.494 2.052-.927 1.056-1.494 2.435-1.494 3.948 0 .467.059.919.16 1.356-.641.414-1.387.644-2.16.644-2.206 0-4-1.794-4-4zm8 2c0-2.206 1.794-4 4-4 1.458 0 2.795.804 3.493 2.052-.926 1.056-1.493 2.436-1.493 3.948 0 .467.059.92.16 1.357-.641.412-1.387.643-2.16.643-2.206 0-4-1.794-4-4zm8 2c0-2.206 1.794-4 4-4s4 1.794 4 4-1.794 4-4 4-4-1.794-4-4zm8.505-3.95c.699-1.247 2.035-2.05 3.493-2.05 2.206 0 4 1.794 4 4s-1.794 4-4 4c-.772 0-1.518-.23-2.159-.642.102-.437.161-.89.161-1.358 0-1.513-.567-2.893-1.495-3.95zm22.197-1.05h-4.794c-.478-2.833-2.942-5-5.91-5-.694 0-1.365.132-1.998.358v-1.358c0-.552.448-1 1-1 1.654 0 3-1.346 3-3v-1c0-.552.448-1 1-1h2.732c2.557 3.464 4.307 7.556 4.97 12zm-14.198-.948c.699-1.248 2.037-2.052 3.494-2.052 2.206 0 4 1.794 4 4s-1.794 4-4 4c-.773 0-1.519-.231-2.16-.643.101-.437.16-.889.16-1.357 0-1.513-.567-2.892-1.494-3.948zm7.599-13.052h-1.103c-1.654 0-3 1.346-3 3v1c0 .552-.448 1-1 1-1.654 0-3 1.346-3 3v2.544c-.403.36-.752.778-1.048 1.239-.873-.496-1.88-.783-2.954-.783-2.066 0-3.957 1.086-5.045 2.783-.595-.338-1.254-.575-1.953-.693v-.532c0-.165.071-.337.186-.449.076-.073.158-.11.242-.11h.007c.68.011 1.323-.233 1.81-.708.486-.475.755-1.11.755-1.791 0-.275.225-.5.5-.5h2.5c1.654 0 3-1.346 3-3v-13.072c3.895 1.559 7.346 3.996 10.103 7.072zm-12.103-7.79v13.79c0 .552-.448 1-1 1h-2.5c-1.379 0-2.5 1.121-2.5 2.5 0 .184-.095.304-.151.358-.055.054-.172.142-.349.142-.005 0-.01-.001-.014-.001-.627.01-1.231.226-1.697.679-.501.489-.789 1.175-.789 1.881v.532c-.7.118-1.359.355-1.954.694-1.088-1.699-2.98-2.785-5.046-2.785-1.074 0-2.081.287-2.954.784-.446-.696-1.019-1.293-1.695-1.749.094-.016.188-.035.284-.035h7.365c1.654 0 3-1.346 3-3 0-1.62-1.318-2.966-2.939-3-.515-.01-1.061-.336-1.061-.92v-3.103c0-.313-.146-.608-.397-.798-1.019-.769-1.603-1.928-1.603-3.179v-3.79c2.529-.786 5.216-1.21 8-1.21s5.471.424 8 1.21zm-28.956 8.79h4.456c.827 0 1.5.673 1.5 1.5s-.673 1.5-1.5 1.5h-2c-1.379 0-2.5 1.121-2.5 2.5 0 .275-.225.5-.5.5h-3.148c.97-2.165 2.213-4.181 3.692-6zm-4.496 8h3.952c1.379 0 2.5-1.121 2.5-2.5 0-.275.225-.5.5-.5h2c1.93 0 3.5-1.57 3.5-3.5s-1.57-3.5-3.5-3.5h-2.662c2.585-2.607 5.699-4.686 9.162-6.072v3.072c0 1.713.723 3.308 2 4.449v2.631c0 1.604 1.327 2.887 3.021 2.92.54.011.979.46.979 1 0 .552-.448 1-1 1h-7.365c-.997 0-1.949.409-2.63 1.091-.33-.056-.664-.091-1.005-.091-3.309 0-6 2.691-6 6 0 1.538.586 2.937 1.54 4h-.542c-2.526 0-4.687 1.57-5.571 3.784-.28-1.568-.427-3.165-.427-4.784 0-3.155.549-6.183 1.548-9zm9.45 28c-1.654 0-3 1.346-3 3v4c0 .552-.448 1-1 1s-1-.448-1-1v-12-7h-2v7c0 .552-.448 1-1 1s-1-.448-1-1v-7c0-2.206 1.794-4 4-4h5 .002c1.042 0 2.049-.282 2.942-.786.178.279.378.541.598.786h-.542c-3.309 0-6 2.691-6 6v7c0 1.654 1.346 3 3 3 .352 0 .686-.072 1-.184v.369c-.314-.113-.648-.185-1-.185zm8 2c-1.654 0-3 1.346-3 3v4c0 .552-.448 1-1 1s-1-.448-1-1v-12-7h-2v7c0 .552-.448 1-1 1s-1-.448-1-1v-7c0-2.206 1.794-4 4-4h5 .002c1.041 0 2.049-.282 2.942-.786.178.279.378.541.598.786h-.54c-3.309 0-6 2.691-6 6v7c0 1.654 1.346 3 3 3 .352 0 .686-.072 1-.184v.369c-.315-.113-.649-.185-1.002-.185zm13.002 9c0 .552-.448 1-1 1s-1-.448-1-1v-4c0-1.654-1.346-3-3-3s-3 1.346-3 3v4c0 .552-.448 1-1 1s-1-.448-1-1v-12-7h-2v7c0 .552-.448 1-1 1s-1-.448-1-1v-7c0-2.206 1.794-4 4-4h10c2.206 0 4 1.794 4 4v7c0 .552-.448 1-1 1s-1-.448-1-1v-7h-2v7zm8-2c0 .552-.448 1-1 1s-1-.448-1-1v-4c0-1.654-1.346-3-3-3-.352 0-.686.072-1 .184v-.369c.314.113.648.185 1 .185 1.654 0 3-1.346 3-3v-7c0-3.309-2.691-6-6-6h-.54c.22-.245.42-.506.598-.785.892.503 1.9.785 2.94.785h.002 5c2.206 0 4 1.794 4 4v7c0 .552-.448 1-1 1s-1-.448-1-1v-7h-2v7zm12-14c0 .552-.448 1-1 1s-1-.448-1-1v-7h-2v7 12c0 .552-.448 1-1 1s-1-.448-1-1v-4c0-1.654-1.346-3-3-3-.352 0-.686.072-1 .184v-.369c.314.113.648.185 1 .185 1.654 0 3-1.346 3-3v-7c0-3.309-2.691-6-6-6h-.542c.22-.245.421-.507.598-.786.893.504 1.901.786 2.942.786h.002 5c2.206 0 4 1.794 4 4z"/></svg>
                            </div>
                            <div class="about-list-item pl-3">
                                <h3 class="widget-title pb-2">Avoid Crowded Places</h3>
                                <p>
                                    The virus propagation is fast in crowded places
                                </p>
                            </div>
                        </li>
                    </ul>
                </div><!-- end note-box -->
            </div><!-- end col-lg-6 -->
            <div class="col-lg-6">
                <div class="note-box do-box">
                   <div class="note-box-header">
                       <div class="icon-element">
                           <i class="la la-check"></i>
                       </div>
                       <h3 class="widget-title note-title text-uppercase"> Things you should to do</h3>
                   </div>
                    <div class="section-block"></div>
                    <ul class="list-items">
                        <li class="d-flex align-items-center mb-4">
                            <div class="about-icon flex-shrink-0">
                                <svg viewBox="-19 -35 624.00103 624" xmlns="http://www.w3.org/2000/svg"><path d="m113.042969 391.878906c9.253906 7.542969 20.023437 13.007813 31.578125 16.03125-16.734375 18.804688-17.367188 46.972656-1.480469 66.5l48.242187 59.183594c17.863282 21.875 50.066407 25.160156 71.972657 7.339844l42.171875-34.378906c21.617187 2.476562 43.3125-3.875 60.1875-17.609376l133.292968-108.675781c7.910157-6.441406 11.996094-16.460937 10.84375-26.597656-1.15625-10.132813-7.390624-18.980469-16.546874-23.476563l11.773437-9.597656c7.984375-6.515625 12.058594-16.679687 10.789063-26.90625-1.269532-10.226562-7.699219-19.082031-17.035157-23.453125 9.863281-11.863281 9.101563-29.269531-1.753906-40.230469-10.863281-10.953124-28.265625-11.871093-40.214844-2.113281l-3.554687 2.898438c-2.558594-9.875-9.972656-17.757813-19.667969-20.929688-9.6875-3.171875-20.328125-1.191406-28.230469 5.257813l-44.800781 36.53125c.667969-13.011719-3.542969-25.804688-11.800781-35.875-6.171875-7.617188-17.34375-8.796875-24.972656-2.644532l-10.953126 8.824219h-4.007812c8.828125-9.5 13.675781-22.019531 13.554688-34.988281-.015626-4.679688-1.898438-9.152344-5.230469-12.4375-3.332031-3.277344-7.839844-5.09375-12.515625-5.035156h-150.046875c-25.976563.054687-49.996094 13.828125-63.160157 36.222656h-40.742187c-28.210937 0-50.734375 23.488281-50.734375 51.695312v76.347657c-.0585938 13.457031 5.273438 26.375 14.804688 35.882812 9.535156 9.5 22.46875 14.792969 35.929687 14.695313h54.429687c2.457032 3.746094 5.085938 5.277344 7.878907 7.539062zm221.839843-165.089844c.050782-.042968.121094-.070312.195313-.058593.066406.003906.128906.039062.175781.09375 12.308594 15.09375 10.050782 37.308593-5.046875 49.621093l-39.074219 31.851563c-2.441406 1.964844-3.644531 5.089844-3.148437 8.1875s2.609375 5.691406 5.546875 6.800781c2.929688 1.109375 6.234375.5625 8.65625-1.4375l39.070312-31.851562.011719-.011719 75.1875-61.300781c3.445313-2.859375 8.164063-3.65625 12.355469-2.074219 4.191406 1.574219 7.210938 5.28125 7.914062 9.707031.703126 4.425782-1.023437 8.886719-4.519531 11.6875l-97.792969 79.730469c-2.464843 1.960937-3.683593 5.09375-3.195312 8.207031.492188 3.109375 2.617188 5.71875 5.566406 6.820313 2.945313 1.109375 6.265625.542969 8.679688-1.476563l97.789062-79.730468 24.65625-20.097657c5.335938-4.277343 13.125-3.449219 17.445313 1.851563 4.320312 5.300781 3.5625 13.09375-1.695313 17.464844l-16.414062 13.382812-.019532.015625-24.65625 20.105469-.019531.019531-81.335937 66.308594c-2.445313 1.96875-3.648438 5.089843-3.152344 8.1875.492188 3.097656 2.609375 5.695312 5.542969 6.800781 2.9375 1.109375 6.242187.5625 8.660156-1.4375l81.347656-66.324219 24.660157-20.105469c5.335937-4.347656 13.1875-3.554687 17.535156 1.785157 4.347656 5.332031 3.550781 13.183593-1.785156 17.53125l-28.757813 23.453125c-.488281.394531-.929687.84375-1.320313 1.332031-.96875.621094-1.902343 1.292969-2.789062 2.015625l-56.703125 46.234375c-2.476563 1.957031-3.707031 5.09375-3.21875 8.214844.488281 3.117187 2.621094 5.730469 5.578125 6.835937 2.957031 1.105469 6.277344.53125 8.6875-1.5l56.710938-46.230468c3.445312-2.859376 8.164062-3.65625 12.355468-2.074219 4.191406 1.578125 7.21875 5.285156 7.917969 9.710937.703125 4.417969-1.023437 8.882813-4.523437 11.679688l-133.296876 108.679687c-13.769531 11.214844-31.621093 16.121094-49.183593 13.519531-2.761719-.804687-5.742188-.199218-7.972657 1.621094l-45.199218 36.847656c-14.417969 11.730469-35.613282 9.574219-47.371094-4.820312l-48.242188-59.171875c-11.746093-14.40625-9.589843-35.617187 4.816407-47.367187l27.445312-22.375c.148438-.125.292969-.242188.429688-.375l8.0625-7.496094c2.320312-2.160156 3.292969-5.410156 2.535156-8.492188-5.007813-20.597656 2.222656-42.246094 18.609375-55.6875zm-284.148437 140.0625c-8.824219.109376-17.320313-3.34375-23.570313-9.566406-6.257812-6.226562-9.738281-14.703125-9.675781-23.523437v-76.347657c0-18.5625 14.675781-34.207031 33.246094-34.207031h33.761719c-1.441406 5.902344-2.183594 11.953125-2.21875 18.035157-.0625 23.414062 9.226562 45.886718 25.808594 62.429687l4.324218 4.390625c3.394532 3.414062 8.917969 3.429688 12.332032.035156 3.417968-3.402344 3.429687-8.925781.03125-12.335937l-4.324219-4.324219c-13.28125-13.144531-20.730469-31.070312-20.683594-49.753906.050781-30.257813 24.617187-54.75 54.871094-54.699219h150.042969c.140624 0 .257812-.160156.257812-.015625-.019531 9.320312-3.75 18.25-10.367188 24.816406-6.617187 6.5625-15.570312 10.222656-24.894531 10.171875h-60.953125c-4.847656.050781-8.75 3.996094-8.742187 8.847657-.015625 16.835937-13.648438 30.496093-30.488281 30.539062-4.832032.015625-8.746094 3.9375-8.746094 8.765625.003906 2.316406.933594 4.523437 2.570312 6.148437 1.640625 1.625 3.867188 2.527344 6.175782 2.507813 23.15625 0 43.003906-16.542969 47.171874-39.320313h74.769532l-30.644532 25.011719c-.320312-.03125-.648437-.039062-.96875-.03125h-19.292968c-4.640625.03125-8.460938 3.652344-8.734375 8.285157-.269531 4.632812 3.097656 8.675781 7.703125 9.253906l-31.949219 26.0625c-20.148437 16.535156-29.914063 42.582031-25.605469 68.285156l-4.25 3.957031-15.621094 12.738281c-28.308593-1-54.160156-22.121093-60.644531-49.925781-1.101562-4.707031-5.800781-7.625-10.503906-6.53125-2.238281.460938-4.191406 1.804688-5.421875 3.730469-1.230469 1.921875-1.628906 4.261719-1.109375 6.484375 1.648437 7.039063 4.304687 13.800781 7.886719 20.085937zm0 0"/><path d="m435.21875 119.089844-33.710938-53.433594c-1.601562-2.535156-4.390624-4.078125-7.390624-4.078125-3.007813 0-5.796876 1.542969-7.398438 4.078125l-33.710938 53.433594c-.175781.273437-.332031.554687-.472656.847656-9.433594 19.429688-4.148437 42.800781 12.726563 56.28125 16.871093 13.484375 40.832031 13.484375 57.703125 0 16.875-13.480469 22.160156-36.851562 12.726562-56.28125-.140625-.292969-.296875-.574219-.472656-.847656zm-20.789062 41.347656c-11.230469 11.195312-29.402344 11.195312-40.632813 0l-6.179687 6.179688 6.179687-6.179688c-8.527344-8.546875-10.816406-21.503906-5.722656-32.449219l26.035156-41.273437 26.046875 41.273437c5.089844 10.945313 2.800781 23.898438-5.726562 32.4375zm0 0"/><path d="m497.160156 26.371094c-3 0-5.792968 1.539062-7.390625 4.078125l-22.476562 35.621093c-.167969.273438-.332031.558594-.472657.851563-6.265624 12.929687-3.660156 28.40625 6.496094 38.570313 11.644532 11.640624 29.992188 13.171874 43.40625 3.625 13.410156-9.550782 17.96875-27.382813 10.78125-42.195313-.140625-.292969-.296875-.578125-.472656-.851563l-22.472656-35.621093c-1.601563-2.539063-4.394532-4.078125-7.398438-4.078125zm11.484375 66.75c-6.339843 6.34375-16.621093 6.34375-22.960937 0-4.78125-4.777344-6.097656-12.015625-3.3125-18.175782l14.789062-23.441406 14.792969 23.441406c2.785156 6.160157 1.46875 13.394532-3.308594 18.175782zm0 0"/><path d="m313.449219 2.96875c-1.601563-2.539062-4.394531-4.078125-7.394531-4.078125-3.003907 0-5.796876 1.539063-7.394532 4.078125l-22.476562 35.621094c-.167969.273437-.332032.558594-.472656.851562-6.265626 12.929688-3.660157 28.40625 6.496093 38.570313 11.644531 11.640625 29.992188 13.171875 43.402344 3.625 13.414063-9.550781 17.972656-27.382813 10.785156-42.195313-.140625-.292968-.296875-.578125-.472656-.851562zm4.085937 62.671875c-6.34375 6.34375-16.621094 6.34375-22.960937 0-4.78125-4.773437-6.097657-12.015625-3.3125-18.171875l14.789062-23.445312 14.792969 23.445312c2.785156 6.15625 1.46875 13.390625-3.308594 18.171875zm0 0"/><path d="m577.042969 193.082031c-15.691407-1.425781-28.128907-13.859375-29.554688-29.558593-.410156-4.503907-4.183593-7.953126-8.707031-7.953126s-8.300781 3.449219-8.707031 7.953126c-1.421875 15.699218-13.859375 28.132812-29.558594 29.558593-4.503906.40625-7.953125 4.183594-7.953125 8.710938 0 4.523437 3.449219 8.300781 7.953125 8.703125 15.699219 1.425781 28.136719 13.863281 29.558594 29.558594.40625 4.503906 4.183593 7.953124 8.707031 7.953124s8.296875-3.449218 8.707031-7.953124c1.425781-15.695313 13.863281-28.132813 29.554688-29.558594 4.507812-.402344 7.957031-4.179688 7.957031-8.703125 0-4.527344-3.449219-8.304688-7.957031-8.710938zm-38.261719 21.382813c-3.429688-4.953125-7.726562-9.246094-12.671875-12.675782 4.945313-3.433593 9.242187-7.726562 12.671875-12.679687 3.433594 4.953125 7.722656 9.246094 12.675781 12.679687-4.949219 3.429688-9.242187 7.722657-12.675781 12.675782zm0 0"/><path d="m539.570312 440.402344c-15.691406-1.425782-28.128906-13.863282-29.554687-29.558594-.410156-4.503906-4.183594-7.953125-8.707031-7.953125-4.523438 0-8.300782 3.449219-8.707032 7.953125-1.421874 15.695312-13.859374 28.132812-29.558593 29.558594-4.5.402344-7.953125 4.179687-7.953125 8.707031 0 4.523437 3.453125 8.300781 7.953125 8.707031 15.699219 1.421875 28.136719 13.859375 29.558593 29.558594.40625 4.503906 4.183594 7.953125 8.707032 7.953125 4.523437 0 8.296875-3.449219 8.707031-7.953125 1.425781-15.699219 13.863281-28.136719 29.554687-29.558594 4.507813-.40625 7.957032-4.183594 7.957032-8.707031 0-4.527344-3.449219-8.304687-7.957032-8.707031zm-38.261718 21.378906c-3.429688-4.953125-7.726563-9.246094-12.671875-12.675781 4.945312-3.429688 9.242187-7.722657 12.671875-12.675781 3.433594 4.953124 7.722656 9.246093 12.675781 12.675781-4.949219 3.429687-9.242187 7.722656-12.675781 12.675781zm0 0"/><path d="m211.484375 59.851562c-.410156-4.503906-4.183594-7.953124-8.707031-7.953124-4.523438 0-8.300782 3.449218-8.707032 7.953124-1.421874 15.695313-13.859374 28.132813-29.558593 29.558594-4.5.402344-7.953125 4.179688-7.953125 8.707032 0 4.523437 3.453125 8.300781 7.953125 8.707031 15.699219 1.421875 28.136719 13.859375 29.558593 29.558593.40625 4.503907 4.183594 7.953126 8.707032 7.953126 4.523437 0 8.296875-3.449219 8.707031-7.953126 1.425781-15.699218 13.863281-28.136718 29.554687-29.558593 4.507813-.40625 7.957032-4.183594 7.957032-8.707031 0-4.527344-3.449219-8.304688-7.957032-8.707032-15.691406-1.425781-28.128906-13.863281-29.554687-29.558594zm-8.707031 50.9375c-3.429688-4.953124-7.726563-9.246093-12.671875-12.675781 4.945312-3.429687 9.242187-7.722656 12.671875-12.675781 3.433594 4.953125 7.722656 9.246094 12.675781 12.675781-4.949219 3.429688-9.242187 7.722657-12.675781 12.675781zm0 0"/></svg>
                            </div>
                            <div class="about-list-item pl-3">
                                <h3 class="widget-title pb-2">Wash Your Hands Regularly For 20sec</h3>
                                <p>
                                    Wash your hands at every chance you get for atleast 20 seconds
                                </p>
                            </div>
                        </li>
                        <li class="d-flex align-items-center mb-4">
                            <div class="about-icon flex-shrink-0">
                                <svg viewBox="-48 0 496 496" xmlns="http://www.w3.org/2000/svg"><path d="m168 152c0 8.835938-7.164062 16-16 16s-16-7.164062-16-16 7.164062-16 16-16 16 7.164062 16 16zm0 0"></path><path d="m264 152c0 8.835938-7.164062 16-16 16s-16-7.164062-16-16 7.164062-16 16-16 16 7.164062 16 16zm0 0"></path><path d="m344.191406 348.761719-52.976562-12.234375-22.773438-15.183594c-3.175781-2.128906-7.394531-1.695312-10.097656 1l-5.433594 5.433594-3.847656-19.25c27.929688-13.6875 49.363281-38.582032 58.3125-68.863282 20.457031-1.753906 36.625-18.761718 36.625-39.664062v-104c0-34.105469-26.808594-62.0625-60.863281-63.902344-19.578125-20.441406-46.761719-32.097656-75.136719-32.097656h-48c-57.34375 0-104 46.65625-104 104v96c0 20.902344 16.167969 37.910156 36.632812 39.65625 8.941407 30.289062 30.375 55.175781 58.3125 68.863281l-3.847656 19.25-5.433594-5.433593c-2.695312-2.695313-6.921874-3.121094-10.097656-1l-22.773437 15.183593-52.96875 12.234375c-32.871094 7.582032-55.824219 36.429688-55.824219 70.15625v69.089844c0 4.425781 3.585938 8 8 8h384c4.414062 0 8-3.574219 8-8v-69.089844c0-33.726562-22.953125-62.574218-55.808594-70.148437zm-79.175781-10.464844 11.886719 7.917969-38.148438 57.226562-19.441406-19.441406zm-66.894531 101.253906c.621094.160157 1.214844.449219 1.878906.449219s1.257812-.289062 1.878906-.449219l5.058594 40.449219h-13.882812zm83.941406-182-17.589844-13.191406c4.757813-9.320313 7.527344-19.121094 7.527344-28.359375 0-6.289062-1.359375-11.496094-3.542969-15.960938l27.542969-22.960937v30.921875c0 18.144531-5.152344 35.0625-13.9375 49.550781zm13.9375-101.304687-37.976562 31.648437c-16.542969-11.824219-43.671876-11.894531-58.023438-11.894531s-41.480469.070312-58.023438 11.894531l-37.976562-31.648437v-56.703125l15.734375 5.898437c51.503906 19.3125 109.027344 19.3125 160.523437 0l15.742188-5.898437zm-152 59.753906c0-11.863281 6.65625-24 56-24s56 12.136719 56 24c0 22.121094-25.089844 56-56 56s-56-33.878906-56-56zm166.894531 6.871094c.648438-4.878906 1.105469-9.816406 1.105469-14.871094v-30.527344c9.289062 3.3125 16 12.113282 16 22.527344 0 10.824219-7.238281 19.886719-17.105469 22.871094zm-150.894531-206.871094h48c25 0 48.910156 10.695312 65.59375 29.335938 1.519531 1.695312 4.125 2.664062 6.40625 2.664062 26.472656 0 48 21.527344 48 48v72.207031c-4.671875-3.535156-10.054688-6.183593-16-7.390625v-72.816406c0-2.625-1.289062-5.078125-3.441406-6.574219-2.160156-1.496093-4.910156-1.839843-7.367188-.914062l-26.542968 9.953125c-47.894532 17.96875-101.390626 17.96875-149.289063 0l-26.542969-9.953125c-2.449218-.925781-5.207031-.574219-7.367187.914062-2.160157 1.496094-3.449219 3.949219-3.449219 6.574219v72.808594c-5.945312 1.214844-11.328125 3.855468-16 7.390625v-64.199219c0-48.519531 39.480469-88 88-88zm-88 184c0-10.414062 6.710938-19.214844 16-22.527344v30.527344c0 5.054688.457031 9.992188 1.105469 14.871094-9.859375-2.984375-17.105469-12.046875-17.105469-22.871094zm32-22.921875 27.542969 22.953125c-2.183594 4.472656-3.542969 9.679688-3.542969 15.96875 0 9.238281 2.769531 19.039062 7.527344 28.359375l-17.589844 13.191406c-8.785156-14.488281-13.9375-31.40625-13.9375-49.550781zm23.304688 93.441406 16.832031-12.625c13.367187 17.144531 33.695312 30.105469 55.863281 30.105469s42.496094-12.960938 55.863281-30.105469l16.832031 12.625c-17.617187 20.457031-43.640624 33.480469-72.695312 33.480469s-55.078125-13.023438-72.695312-33.480469zm38.710937 44.175781c10.726563 3.425782 22.136719 5.304688 33.984375 5.304688s23.257812-1.878906 33.984375-5.304688l5.335937 26.679688-36.976562 36.976562c-1.558594 1.550782-2.34375 3.601563-2.34375 5.648438s.785156 4.097656 2.34375 5.65625l11.097656 11.097656-13.441406 17.910156-13.441406-17.910156 11.097656-11.097656c1.558594-1.558594 2.34375-3.609375 2.34375-5.65625s-.785156-4.097656-2.34375-5.65625l-36.976562-36.976562zm-31.03125 23.601563 45.703125 45.703125-19.449219 19.449219-38.148437-57.226563zm-118.984375 80.613281c0-26.230468 17.847656-48.671875 43.40625-54.558594l49-11.3125 44.929688 67.402344c1.328124 1.992188 3.488281 3.285156 5.863281 3.519532.28125.023437.535156.039062.800781.039062 2.113281 0 4.152344-.832031 5.65625-2.34375l9.480469-9.480469 8.808593 11.753907-7.007812 56.070312h-160.9375zm368 61.089844h-160.9375l-7.007812-56.070312 8.808593-11.753907 9.480469 9.480469c1.503906 1.511719 3.542969 2.34375 5.65625 2.34375.265625 0 .519531-.015625.792969-.039062 2.382812-.234376 4.535156-1.527344 5.863281-3.519532l44.929688-67.402344 49.007812 11.3125c25.558594 5.886719 43.40625 28.328126 43.40625 54.558594zm0 0"></path></svg>
                            </div>
                            <div class="about-list-item pl-3">
                                <h3 class="widget-title pb-2">Wear a Mask if Available</h3>
                                <p>
                                    Masks protects you and others from the virus
                                </p>
                            </div>
                        </li>
                        <li class="d-flex align-items-center mb-4">
                            <div class="about-icon flex-shrink-0">
                                <svg id="_x34_0" enable-background="new 0 0 64 64" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><g><path d="m31 22h2v4h-2z"></path><path d="m39.555 23.168-1.752-1.168 1.752-1.168-1.11-1.664-4.248 2.832 4.248 2.832z"></path><path d="m25.555 24.832 4.248-2.832-4.248-2.832-1.11 1.664 1.752 1.168-1.752 1.168z"></path><path d="m54 31h-.879c-2.698 0-5.039 1.776-5.832 4.272-.01-.002-.02-.005-.03-.007l-6.488-1.145c-.124-.339-.273-.665-.433-.983 1.587-1.77 2.688-3.867 3.24-6.137h.422c2.206 0 4-1.794 4-4 0-.027-.007-.052-.008-.079 3.391-.488 6.008-3.424 6.008-6.971 0-.192.072-.365.207-.5 1.156-1.156 1.793-2.693 1.793-4.329 0-3.375-2.746-6.121-6.121-6.121h-.879c-1.542 0-3.023.409-4.354 1.15-2.636-3.144-6.588-5.15-11.002-5.15h-3.288c-3.563 0-6.82 1.313-9.333 3.47-1.237-2.115-3.498-3.47-6.023-3.47-3.86 0-7 3.163-7 7.05 0 .192-.072.365-.207.5-1.156 1.156-1.793 2.693-1.793 4.329 0 3.375 2.746 6.121 6.121 6.121h.879c1.149 0 2.28-.23 3.337-.654.035.163.051.33.091.492l.429 1.717c-.528.678-.857 1.52-.857 2.445 0 2.206 1.794 4 4 4h.422c.552 2.27 1.653 4.367 3.24 6.137-.16.318-.309.644-.433.983l-6.274 1.107c-.386-3.497-3.356-6.227-6.955-6.227-3.86 0-7 3.163-7 7.05 0 .192-.072.365-.207.5-1.156 1.156-1.793 2.693-1.793 4.329 0 3.01 2.186 5.513 5.052 6.021-.034.385-.052.775-.052 1.168v7.568c0 4.061 3.303 7.364 7.364 7.364h44.636v-14.296c2.941-.871 5-3.563 5-6.704v-2c0-4.962-4.038-9-9-9zm-19.664 14.078.646 5.813-.024.072c-.406 1.218-1.542 2.037-2.828 2.037-.999 0-1.926-.496-2.48-1.328-.254-.381-.583-.697-.949-.959zm2.368 3.197-.557-5.008.525-.525c.856-.856 1.328-1.995 1.328-3.206 0-2.501-2.035-4.536-4.536-4.536-1.211 0-2.351.472-3.207 1.329l-.671.671h-.172c-1.179 0-2.288.459-3.121 1.293-.834.833-1.293 1.942-1.293 3.121v.172l-1.464 1.464 1.232-6.982c.621-3.516 3.662-6.068 7.232-6.068s6.611 2.552 7.231 6.067l1.756 9.947c.009.049.013.098.013.147 0 .463-.376.839-.839.839-1.302 0-2.51.478-3.457 1.275zm7.296-23.275h-.068c.045-.464.068-.932.068-1.403v-2.597c1.103 0 2 .897 2 2s-.897 2-2 2zm3.443-13.56c.466-.285 1.002-.44 1.557-.44h.879c.065 0 .121.056.086.207-1.267 1.267-1.965 2.951-1.965 4.743 0 .085-.032.158-.051.236.016-.277.051-.553.051-.83 0-1.359-.202-2.669-.557-3.916zm1.557-4.44h.879c2.272 0 4.121 1.849 4.121 4.121 0 1.101-.429 2.136-1.208 2.915-.511.511-.792 1.191-.792 1.914 0 2.639-2.022 4.788-4.58 5.007-.084-.141-.177-.274-.278-.403l.403-1.611c1.394-.261 2.455-1.501 2.455-2.993 0-1.257.49-2.439 1.38-3.33.4-.4.62-.933.62-1.499 0-1.169-.952-2.121-2.121-2.121h-.879c-.788 0-1.546.204-2.236.552-.274-.617-.595-1.208-.95-1.776.982-.505 2.065-.776 3.186-.776zm-36 10h-.879c-2.272 0-4.121-1.849-4.121-4.121 0-1.101.429-2.136 1.208-2.915.511-.511.792-1.191.792-1.914 0-2.784 2.243-5.05 5-5.05 1.983 0 3.739 1.167 4.543 2.943-.578.663-1.093 1.379-1.543 2.14v-.083c0-1.654-1.346-3-3-3s-3 1.368-3 3.05c0 1.257-.49 2.439-1.38 3.33-.4.4-.62.933-.62 1.499 0 1.169.952 2.121 2.121 2.121h.879c1.164 0 2.222-.416 3.073-1.086-.047.474-.073.955-.073 1.442 0 .309.037.616.057.924-.95.464-1.992.72-3.057.72zm3-9v2c0 1.654-1.346 3-3 3h-.879c-.065 0-.121-.056-.086-.207 1.267-1.267 1.965-2.951 1.965-4.743 0-.589.439-1.05 1-1.05.551 0 1 .449 1 1zm4 17c-1.103 0-2-.897-2-2s.897-2 2-2v2.597c0 .472.023.94.068 1.403zm.162-6h-.162c-.495 0-.964.102-1.402.267l-.229-.915c-.245-.978-.369-1.986-.369-2.996 0-6.813 5.543-12.356 12.356-12.356h3.288c6.813 0 12.356 5.543 12.356 12.356 0 1.01-.124 2.018-.369 2.997l-.229.915c-.438-.166-.907-.268-1.402-.268h-.274c-.411-1.365-1.274-2.567-2.494-3.38-.536-.357-1.122-.628-1.74-.805l-4.736-1.353c.144-.466.244-.95.244-1.462v-5h-2v5c0 1.654-1.346 3-3 3h-4.697c-1.05 0-2.067.308-2.941.891-1.109.739-1.882 1.85-2.2 3.109zm1.838 4.597v-3.294c0-1.107.55-2.134 1.471-2.748.544-.363 1.178-.555 1.832-.555h4.697c1.507 0 2.844-.684 3.762-1.742l5.181 1.48c.419.12.816.303 1.18.546 1.175.783 1.877 2.094 1.877 3.508v2.805c0 2.88-.983 5.61-2.788 7.807-1.726-2.096-4.332-3.404-7.212-3.404s-5.486 1.308-7.212 3.403c-1.805-2.196-2.788-4.926-2.788-7.806zm.706 12.647-1.636 9.272-5.07 5.07v-3.586h-2v4h-6v-2.932c0-5.348 3.822-9.903 9.088-10.833zm-14.706 4.756h-.879c-.065 0-.121-.056-.086-.207 1.267-1.267 1.965-2.951 1.965-4.743 0-.589.439-1.05 1-1.05.551 0 1 .449 1 1v1.829c-1.137.888-2.115 1.956-2.898 3.162-.034.002-.067.009-.102.009zm-5-.121c0-1.101.429-2.136 1.208-2.915.511-.511.792-1.191.792-1.914 0-2.784 2.243-5.05 5-5.05 2.661 0 4.823 2.095 4.972 4.719-.686.223-1.344.499-1.972.825v-.544c0-1.654-1.346-3-3-3s-3 1.368-3 3.05c0 1.257-.49 2.439-1.38 3.33-.4.4-.62.933-.62 1.499 0 1.141.909 2.069 2.04 2.113-.264.623-.482 1.268-.648 1.935-1.925-.347-3.392-2.025-3.392-4.048zm5 14.757v-2.636h5.586l-2.293 2.293 1.414 1.414 14.293-14.293v-1c0-.635.258-1.258.707-1.707s1.072-.707 1.707-.707h1l1.257-1.257c.479-.479 1.116-.743 1.793-.743 1.398 0 2.536 1.138 2.536 2.536 0 .677-.264 1.314-.743 1.793l-18.1 18.1c-1.013 1.013-2.36 1.571-3.793 1.571-2.958 0-5.364-2.406-5.364-5.364zm10.571 5.207 8.678-8.678c.295.135.554.343.737.617.926 1.389 2.475 2.218 4.144 2.218 2.148 0 4.047-1.369 4.725-3.405l.094-.28c.462-1.385 1.752-2.315 3.212-2.315 1.566 0 2.839-1.273 2.839-2.839 0-.166-.014-.331-.043-.494l-1.663-9.424 5.618.991c5.266.931 9.088 5.486 9.088 10.834v2.932h-6v-4h-2v14h-29.602c.057-.053.117-.101.173-.157zm38.429-18.843c0 .221-.082.423-.208.591-.625-1.348-1.476-2.573-2.509-3.626-.322-.646-.734-1.249-1.283-1.844 0-.065.056-.121.121-.121h.879c1.654 0 3 1.346 3 3zm-7 19v-8h6v8zm11-19c0 2.066-1.253 3.851-3.091 4.608-.078-.694-.203-1.375-.386-2.034.886-.528 1.477-1.493 1.477-2.574v-2c0-2.757-2.243-5-5-5h-.879c-.983 0-1.805.676-2.044 1.585-.59-.312-1.206-.581-1.849-.802.562-1.631 2.11-2.783 3.893-2.783h.879c3.86 0 7 3.14 7 7z"></path><path d="m57 23c0-1.103.897-2 2-2s2 .897 2 2-.897 2-2 2v2c2.206 0 4-1.794 4-4s-1.794-4-4-4-4 1.794-4 4z"></path><path d="m4 25c1.654 0 3-1.346 3-3s-1.346-3-3-3-3 1.346-3 3 1.346 3 3 3zm0-4c.551 0 1 .449 1 1s-.449 1-1 1-1-.449-1-1 .449-1 1-1z"></path></g></svg>
                            </div>
                            <div class="about-list-item pl-3">
                                <h3 class="widget-title pb-2">Cover Nose When Sneezing</h3>
                                <p>
                                    Help in stopping the spread of the virus by covering your sneeze
                                </p>
                            </div>
                        </li>
                        <li class="d-flex align-items-center mb-4">
                            <div class="about-icon flex-shrink-0">
                                <svg id="Capa_6" enable-background="new 0 0 512 512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><g><path d="m421.665 41.411h-83.614v-9.064c0-4.142-3.357-7.5-7.5-7.5h-37.095c-6.256-14.882-20.914-24.847-37.455-24.847s-31.199 9.965-37.455 24.847h-37.094c-4.143 0-7.5 3.358-7.5 7.5v9.064h-83.617c-13.271 0-24.067 10.796-24.067 24.067v42.988c0 4.142 3.357 7.5 7.5 7.5s7.5-3.358 7.5-7.5v-42.987c0-5 4.067-9.067 9.067-9.067h83.617v18.136h-67.052c-4.143 0-7.5 3.358-7.5 7.5v389.32c0 4.142 3.357 7.5 7.5 7.5h298.198c4.143 0 7.5-3.358 7.5-7.5v-389.321c0-4.142-3.357-7.5-7.5-7.5h-67.048v-18.136h83.614c5 0 9.067 4.067 9.067 9.067v422.454c0 5-4.067 9.067-9.067 9.067h-331.329c-5 0-9.067-4.067-9.067-9.067v-349.465c0-4.142-3.357-7.5-7.5-7.5s-7.5 3.358-7.5 7.5v349.466c0 13.271 10.797 24.067 24.067 24.067h331.33c13.27 0 24.067-10.796 24.067-24.067v-422.454c0-13.271-10.797-24.068-24.067-24.068zm-232.713-1.564h34.961c3.422 0 6.41-2.316 7.263-5.63 2.913-11.315 13.122-19.217 24.825-19.217s21.912 7.902 24.825 19.217c.853 3.313 3.841 5.63 7.263 5.63h34.962v51.269h-59.55v-9.066c0-4.142-3.357-7.5-7.5-7.5s-7.5 3.358-7.5 7.5v9.066h-59.549zm208.647 49.7v374.32h-283.199v-374.32h59.552v9.069c0 4.142 3.357 7.5 7.5 7.5h149.099c4.143 0 7.5-3.358 7.5-7.5v-9.069z"/><path d="m256 314.767h-33.133c-4.143 0-7.5 3.358-7.5 7.5s3.357 7.5 7.5 7.5h33.133c4.143 0 7.5-3.358 7.5-7.5s-3.357-7.5-7.5-7.5z"/><path d="m289.133 347.897h-66.267c-4.143 0-7.5 3.358-7.5 7.5s3.357 7.5 7.5 7.5h66.267c4.143 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5z"/><path d="m306.482 355.399c0 4.142 3.357 7.5 7.5 7.5h24.852c4.143 0 7.5-3.358 7.5-7.5s-3.357-7.5-7.5-7.5h-24.852c-4.142 0-7.5 3.358-7.5 7.5z"/><path d="m192.196 226.022 58.501 58.502c1.407 1.407 3.314 2.197 5.304 2.197 1.989 0 3.896-.79 5.304-2.197l58.571-58.572c19.071-19.073 19.071-50.105 0-69.178-9.24-9.239-21.524-14.328-34.59-14.328-10.703 0-20.88 3.414-29.29 9.727-19.143-14.325-46.479-12.793-63.872 4.599-19.071 19.072-19.071 50.106 0 69.178.024.025.048.049.072.072zm10.534-58.642c13.224-13.224 34.741-13.224 47.965-.001.232.232.48.449.74.648 1.303 1 2.905 1.55 4.566 1.55 1.989 0 3.896-.79 5.304-2.197 6.405-6.406 14.923-9.934 23.981-9.934 9.06 0 17.576 3.528 23.982 9.934 13.225 13.224 13.224 34.741 0 47.965l-53.268 53.269-53.268-53.269c-.021-.021-.044-.043-.065-.064-13.16-13.229-13.139-34.698.063-47.901z"/><path d="m189.733 314.767h-33.133c-4.143 0-7.5 3.358-7.5 7.5v33.133c0 4.142 3.357 7.5 7.5 7.5h33.134c4.143 0 7.5-3.358 7.5-7.5v-33.133c-.001-4.142-3.358-7.5-7.501-7.5zm-7.5 33.133h-18.133v-18.133h18.134v18.133z"/><path d="m222.867 396.034h33.133c4.143 0 7.5-3.358 7.5-7.5s-3.357-7.5-7.5-7.5h-33.133c-4.143 0-7.5 3.358-7.5 7.5s3.358 7.5 7.5 7.5z"/><path d="m289.133 414.164h-66.267c-4.143 0-7.5 3.358-7.5 7.5s3.357 7.5 7.5 7.5h66.267c4.143 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5z"/><path d="m338.834 414.166h-24.852c-4.143 0-7.5 3.358-7.5 7.5s3.357 7.5 7.5 7.5h24.852c4.143 0 7.5-3.358 7.5-7.5s-3.357-7.5-7.5-7.5z"/><path d="m189.733 381.033h-33.133c-4.143 0-7.5 3.358-7.5 7.5v33.133c0 4.142 3.357 7.5 7.5 7.5h33.134c4.143 0 7.5-3.358 7.5-7.5v-33.133c-.001-4.142-3.358-7.5-7.501-7.5zm-7.5 33.134h-18.133v-18.133h18.134v18.133z"/></g></svg>
                            </div>
                            <div class="about-list-item pl-3">
                                <h3 class="widget-title pb-2">Seek Medical Care Regularly</h3>
                                <p>
                                    If you notice any symptoms seek a medical care before your state worsens
                                </p>
                            </div>
                        </li>
                    </ul>
                </div><!-- end note-box -->
            </div><!-- end col-lg-6 -->
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end service-area -->
<!--======================================
        END SERVICE AREA
======================================-->





<!--======================================
        START MAP AREA
======================================-->
<section class="map-area overflow-hidden section-padding">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="section-heading text-center">
                    <h5 class="section__meta">Coronavirus: tracker</h5>
                    <h2 class="section__title padding-bottom-15px">CovTunisie infected users map</h2>
                    <div class="section-divider"></div>
                </div><!-- end section-heading -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
        <div class="row mt-5">
            <div class="col-lg-12">
                <div class="map-container">
                    <div class="maps" id="mapid"></div>
                    <h3 class="widget-title pt-3">Note: If you want to see the data you hover on the red circle bubble</h3>
                </div>
            </div>
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end map-area -->
<!--======================================
        END MAP AREA
======================================-->





<section class="map-area overflow-hidden section-padding">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="section-heading text-center">
                    <h5 class="section__meta">Report an active case</h5>
                    <h2 class="section__title padding-bottom-15px">Do you know the location of an active case?</h2>
                    <div class="section-divider"></div>
                </div><!-- end section-heading -->
            </div><!-- end col-lg-12 -->
        </div><!-- end row -->
        <div class="row mt-5">
            <div class="col-lg-7">
            <div id="reportmap" style="height:100%;width:100%"></div>
            </div><!-- end col-lg-7 -->
            <div class="col-lg-5">
                <div class="user-form">
                    <div class="section-heading">
                        <h5 class="section__meta">Report case</h5>
                        <h2 class="section__title font-size-20 font-weight-medium">Enter the active case info and select a location on map!</h2>
                    </div><!-- end section-heading -->
                    <div class="contact-form-action mt-4">
                    <form method="POST" name="reportform" id="reportform" action="reportCase.php">
                            <div class="input-box">
                                <div class="form-group" data-validate = "Field can not be empty">
                                    <input class="form-control" type="text" name="name" placeholder="Infected Case Name" >
                                    <span class="la la-user input-icon"></span>
                                </div><!-- end form-group -->
                            </div>
                            
                            <div class="input-box">
                                <div class="form-group" data-validate = "Field can not be empty">
                                    <input class="form-control" type="text" name="testRefId" placeholder="Test Reference ID" >
                                    <span class="la la-book input-icon"></span>
                                </div><!-- end form-group -->
                            </div>
                            <div class="input-box">
                                <div class="form-group" data-validate = "Field can not be empty">
                                    <textarea class="message-control form-control" name="extraInfo" placeholder="Extra information" ></textarea>
                                    <span class="la la-pencil input-icon"></span>
                                </div><!-- end form-group -->
                            </div>
                            <div class="input-box">
                                <div class="form-group" data-validate = "Please select a location on the map">
                                    <input class="form-control" type="text" id="addressinput" name="address" placeholder="Address"  readonly="readonly" >
                                    <span class="la la-map-marker input-icon"></span>
                                </div><!-- end form-group -->
                            </div>



                            <input type="hidden" id="reportGeoLong" name="reportGeoLong" value="">
                            <input type="hidden" id="reportGeoLat" name="reportGeoLat" value="">


                            
                        </form>
                        <div class="btn-box">
                                <div class="form-group text-center mb-0">
                                    <button class="template-btn border-0 w-100" onclick="submitReport()">Report case</button>
                                </div><!-- end form-group -->
                            </div>
                    </div><!-- end contact-form-action -->
                </div>
            </div>
        </div><!-- end row -->
    </div><!-- end container -->
</section><!-- end accordion-area -->
<!--======================================
        END ACCORDION AREA
======================================-->



<!-- ================================
         END FOOTER AREA
================================= -->
<section class="footer-area padding-top-90px">
    <div id="particles-bg"></div>
    <div class="container">
        <div class="row">
            <div class="col-lg-3 column-td-6">
                <div class="footer-widget">
                    <a href="index.html">
                        <img src="images/logo.png" alt="footer logo" class="footer__logo">
                    </a>
                    <p class="padding-top-30px padding-bottom-20px">
                        Quis ipsum suspendisse ultrices gravida. Risus commodo viverra maecenas accumsan.
                        sed do eiusmod tempor incididunt ut labor
                    </p>
                    <h3 class="widget-title font-size-16 font-weight-medium pb-3 mb-0">Connect with us</h3>
                    <ul class="social-links">
                        <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                        <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                        <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                        <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                    </ul>
                </div><!-- end footer-widget -->
            </div><!-- end col-lg-3 -->
            <div class="col-lg-3 column-td-6">
                <div class="footer-widget">
                    <h3 class="widget-title">Privacy & Tos</h3>
                    <div class="section-divider"></div>
                    <ul class="list-items">
                        <li><a href="#">Advertiser agreement</a></li>
                        <li><a href="#">Developer agreement</a></li>
                        <li><a href="#">Privacy Policy</a></li>
                        <li><a href="#">Technology Privacy</a></li>
                        <li><a href="#">Acceptable user policy</a></li>
                    </ul>
                </div><!-- end footer-widget -->
            </div><!-- end col-lg-3 -->
            <div class="col-lg-3 column-td-6">
                <div class="footer-widget">
                    <h3 class="widget-title">Useful links</h3>
                    <div class="section-divider"></div>
                    <ul class="list-items">
                        <li><a href="#">Situation Reports</a></li>
                        <li><a href="#">Advice For Public</a></li>
                        <li><a href="#">Information</a></li>
                        <li><a href="#">Coronavirus Symptoms</a></li>
                        <li><a href="#">Donor & Partners</a></li>
                    </ul>
                </div><!-- end footer-widget -->
            </div><!-- end col-lg-3 -->
            <div class="col-lg-3 column-td-6">
                <div class="footer-widget">
                    <h3 class="widget-title">Contact Us</h3>
                    <div class="section-divider"></div>
                    <ul class="list-items">
                        <li><span class="mr-1 font-weight-medium text-color">Address:</span>2750 Quadra Street Victoria, Canada.</li>
                        <li><span class="mr-1 font-weight-medium text-color">Email:</span><a href="#">hello@example.com</a></li>
                        <li><span class="mr-1 font-weight-medium text-color">Phone:</span><a href="#">+44 587 154756</a></li>
                        <li><span class="mr-1 font-weight-medium text-color">Fax:</span><a href="#">+55 785 4578964</a></li>
                    </ul>
                </div><!-- end footer-widget -->
            </div><!-- end col-lg-3 -->
        </div><!-- end row -->
        <div class="row">
            <div class="col-lg-12">
                <div class="copyright-content text-center">
                    <p class="copy__desc">&copy; 2020 Costar. All Rights Reserved. By <a href="https://themeforest.net/user/techydevs/portfolio">TechyDevs.</a></p>
                </div>
            </div><!-- end col-lg-12 -->
        </div><!-- end copyright-content -->
    </div><!-- end container -->
</section><!-- end footer-area -->
<!-- ================================
          END FOOTER AREA
================================= -->

<!-- start scroll top -->
<div id="scroll-top">
    <i class="fa fa-angle-up" title="Go top"></i>
</div>
<!-- end scroll top -->


<!-- template js files -->
<script src="js/jquery-3.4.1.min.js"></script>
<script src="js/popper.min.js"></script>
<script src="js/bootstrap.min.js"></script>
<script src="js/owl.carousel.min.js"></script>
<script src="js/magnific-popup.min.js"></script>
<script src="js/purecounter.js"></script>
<script src="js/particles.min.js"></script>
<script src="js/particlesRun.js"></script>
<script src="js/jquery.ripples-min.js"></script>
<script src="js/smooth-scrolling.js"></script>
<script src="js/main.js"></script>
<script src="https://unpkg.com/leaflet@1.7.1/dist/leaflet.js"
   integrity="sha512-XQoYMqMTK8LvdxXYG3nZ448hOEQiglfqkJs1NOQV44cWnUrBc8PkAOcXy20w0vlaXaVUearIOBhiXZ5V3ynxwA=="
   crossorigin=""></script>





   <script>

//var mymap = L.map('mapid').setView([33.8869, 9.5375], 6);
var bounds = new L.LatLngBounds(new L.LatLng(38.352693, 6.682708), new L.LatLng(28.878755, 12.508503));

var mymap = new L.Map('mapid', {
  center: [33.8869, 9.5375],
  zoom: 7,
  minZoom: 7,
  maxBounds: bounds,
  maxBoundsViscosity: 0.5
});


var reportmap = new L.Map('reportmap', {
  center: [33.8869, 9.5375],
  zoom: 7,
  minZoom: 7,
  maxBounds: bounds,
  maxBoundsViscosity: 0.5
});



var marker;
reportmap.on('click', function(e) {
    if(marker)
    reportmap.removeLayer(marker);
    console.log(e.latlng); // e is an event object (MouseEvent in this case)
    marker = L.marker(e.latlng).addTo(reportmap);
    
    document.getElementById("addressinput").value = "";
    


    $.ajax({
            url: "https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat="+ e.latlng.lat +"&lon="+e.latlng.lng,
            type: 'GET',
            dataType: 'json',
            headers: {
                'accept-language': 'fr'
            },
            contentType: 'application/json; charset=utf-8',
            success: function (data) {
                console.log(data);
                document.getElementById("addressinput").value = data["display_name"];
                document.getElementById("reportGeoLong").value = data["lon"];
                document.getElementById("reportGeoLat").value = data["lat"];
                hideValidate(document.getElementById("addressinput"));
            },
            error: function (error) {
                
            }
        });
//marker.getLatLng()




});

//https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=35.523285&lon=9.774529


L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}', {
    attribution: 'Map data &copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors, <a href="https://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="https://www.mapbox.com/">Mapbox</a>',
    maxZoom: 18,
    id: 'mapbox/streets-v11',
    tileSize: 512,
    zoomOffset: -1,
    accessToken: 'pk.eyJ1IjoiaXllZGJoZCIsImEiOiJja2k3cDZneTAxa25qMnRxc245ZjVid3RiIn0.s-1nkknLeUOcJwGG-lfuZA'
}).addTo(mymap);



L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}', {
    attribution: 'Map data &copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors, <a href="https://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="https://www.mapbox.com/">Mapbox</a>',
    maxZoom: 18,
    id: 'mapbox/streets-v11',
    tileSize: 512,
    zoomOffset: -1,
    accessToken: 'pk.eyJ1IjoiaXllZGJoZCIsImEiOiJja2k3cDZneTAxa25qMnRxc245ZjVid3RiIn0.s-1nkknLeUOcJwGG-lfuZA'
}).addTo(reportmap);

    
    
    
var theInfected = <?php echo json_encode($infectedRows); ?>;
var theReported = <?php echo json_encode($reportedRows); ?>;


    
for(var s=0;s<theInfected.length;s++)
    {
        var circle = L.circle([parseFloat(theInfected[s]["geo_lat"]), parseFloat(theInfected[s]["geo_long"])], {
            color: 'red',
            fillColor: '#f03',
            fillOpacity: 0.5,
            radius: 2000
        }).addTo(mymap);
        circle.bindPopup(theInfected[s]["prenom"] + " " +  theInfected[s]["nom"] + " marked himself infected");

    }

    for(var s=0;s<theReported.length;s++)
    {
        console.log([parseFloat(theReported[s]["reportedLat"]), parseFloat(theReported[s]["reportedLong"])]);
        var circle = L.circle([parseFloat(theReported[s]["reportedLat"]), parseFloat(theReported[s]["reportedLong"])], {
            color: theReported[s]["status"]==0?'orange':"green",
            fillColor: theReported[s]["status"]==0?'#ff9447':'#00FF00',
            fillOpacity: 0.5,
            radius: 1500
        }).addTo(mymap);
        circle.bindPopup(theReported[s]["reportedName"]  + " was reported to be a carrier");

    }




    var input = $('.form-control');

    function submitReport() {
        //e.preventDefault();

        var check = true;

        for(var i=0; i<input.length; i++) {
            if(validate(input[i]) == false){
                showValidate(input[i]);
                check=false;
            }
        }

        if(!check)return;

    document.getElementById("reportform").submit();




    }

    $('.form-control').each(function(){
        $(this).focus(function(){
            console.log("hiding validate");
        hideValidate(this);
        });
    });

    function validate (input) {
        if($(input).attr('type') == 'email' || $(input).attr('name') == 'email') {
            if($(input).val().trim().match(/^([a-zA-Z0-9_\-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{1,5}|[0-9]{1,3})(\]?)$/) == null) {
                return false;
            }
        }
        else {
            if($(input).val().trim() == ''){
                return false;
            }
        }
    }

    function showValidate(input) {
        var thisAlert = $(input).parent();

        $(thisAlert).addClass('alert-validate');
    }

    function hideValidate(input) {
        var thisAlert = $(input).parent();

        $(thisAlert).removeClass('alert-validate');
    }


    </script>







</body>

<!-- Mirrored from techydevs.com/demos/themes/html/costar/prevention.html by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 01 Dec 2020 23:03:15 GMT -->
</html>