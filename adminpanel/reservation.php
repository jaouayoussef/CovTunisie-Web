...
$list = $mysqli->query("SELECT * FROM `reservation`") or die("db error");
...
<li class="sidebar-item"><a class="sidebar-link" href="commandes.php">Commandes</a></li>
					...
					<li class="sidebar-item">
						<a data-target="#ui3" data-toggle="collapse" class="sidebar-link">
							<i class="align-middle" data-feather="home"></i> <span class="align-middle">Cliniques</span>
						</a>
						<ul id="ui3" class="sidebar-dropdown list-unstyled collapse show" data-parent="#sidebar">
						...
						<li class="sidebar-item active"><a class="sidebar-link" href="reservation.php">Reservations</a></li>
									...
									<table id="datatables-reponsive" class="table table-striped" style="width:100%">
										...
										<tbody>
											...
											echo '<tr>
												...
												<td id="datecreation">' . $row["datecreation"] . '</td>
												<td id="etat">' . (($row["etat"] != "Confirmé")?'<span class="badge bg-warning">'. $row["etat"] .'</span>':'<span class="badge bg-success">'. $row["etat"] . '</span>') . '</td>
												<td id="dateDeTest">' . $row["dateDeTest"] . '</td>
												<td id="clinique">' . $row["clinique"] . '</td>
												<td id="userid">' . $row["userid"] . '</td>
                                                <td id="actions">
                                                    
                                                <button id="accept" class="btn btn-success reportadminbutton" action="Confirmé"><i class="fas fa-check"></i></button>
                                                <button id="decline" class="btn btn-danger reportadminbutton" action="delete"><i class="fas fa-times"></i></button>
                                                <button id="pend" class="btn btn-warning reportadminbutton" action="En Cours"><i class="fas fa-exclamation"></i></button>
...
<script>
  ...
  $('.reportadminbutton').each(function(btn){
        $(this).click(function(){
			...
			var statuschange = this.getAttribute("action");

			setTimeout(function()
			{

				$.get( "changereservationstatus.php?reportid="+$(this).parent().siblings("#id").text() + "&status=" + statuschange, function( data ) {
					...
					if(statuschange == "delete")
					{
						...
						$(this).parent().siblings("#etat").html('<span class="badge bg-danger">Deleted</span>');
						$(this).remove();
					...
					else
					{
						...
						if(statuschange == "Confirmé")
						{
							$(this).parent().siblings("#etat").html('<span class="badge bg-success">' + statuschange + '</span>');
						}
						else
						{
							$(this).parent().siblings("#etat").html('<span class="badge bg-warning">' + statuschange + '</span>');
